...
DYN_DATA::DYN_DATA(iSTEPS& toolkit)
{
...
iSTEPS& DYN_DATA::get_toolkit() const
{
    return *toolkit;
}
...
void DYN_DATA::add_model(MODEL* model)
{
...
size_t DYN_DATA::get_model_size(MODEL* model) const
{
    ...
    if(model_name=="VSCHVDCP0") return sizeof(VSCHVDCP0);
    if(model_name=="VSCHVDCC0") return sizeof(VSCHVDCC0);
    ...
    ostringstream osstream;
    osstream<<"Model '"<<model_name<<"' is not supported in DYN_DATA::"<<__FUNCTION__<<". Fatal error may occur.\n"
            <<"Update DYN_DATA::"<<__FUNCTION__<<"() is required to include the new model.\n"
...
void DYN_DATA::common_set_model(MODEL* model, size_t model_size)
{
...
void DYN_DATA::shrink_model_starting_position_table_at_position(void *pos)
{
    size_t n = model_starting_position_table.size();
...
bool DYN_DATA::load_related_model_is_to_update(MODEL* old_model, MODEL* new_model)
{
...
void DYN_DATA::remove_the_last_model()
{
    size_t n = model_starting_position_table.size();
    switch(n)
    {
...
void DYN_DATA::check_device_model_minimum_time_constants()
{
    vector<DEVICE_MODEL_MIN_T> mint_list;
    size_t max_record_count = 100;
...
size_t DYN_DATA::get_memory_usage()
{
    ostringstream osstream;
    ...
    toolkit->show_information(osstream);
    return warehouse_capacity*sizeof(char)+model_starting_position_table.capacity()*sizeof(size_t);
}
