...
using namespace std;
FILEIRRAD::FILEIRRAD(iSTEPS& toolkit) : PV_IRRADIANCE_MODEL(toolkit)
{
    clear();
}
...
{
    for(size_t i=0; i<STEPS_MAX_SOLAR_IRRADIANCE_RECORD_SIZE; ++i)
    {
...
size_t FILEIRRAD::get_solar_irradiance_record_count() const
{
    size_t n = 0;
    for(size_t i=0; i<STEPS_MAX_SOLAR_IRRADIANCE_RECORD_SIZE; ++i)
    {
...
void FILEIRRAD::set_previous_position(size_t pos)
{
    size_t n = get_solar_irradiance_record_count();
    if(n>=1)
    {
        if(pos<=n-1)
...
size_t FILEIRRAD::get_previous_position() const
{
...
{
    iSTEPS& toolkit = get_toolkit();
    toolkit.show_information(get_standard_psse_string());
}
