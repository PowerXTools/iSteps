...
PUFLS::PUFLS(iSTEPS& toolkit) : LOAD_FREQUENCY_RELAY_MODEL(toolkit),
...
bool PUFLS::setup_model_with_steps_string_vector(vector<string>& data)
{
    ...
    if(n>=12)
    {
        ...
        if(model_name=="PUFLSAL" or model_name=="PUFLSBL" or model_name=="PUFLSAR" or model_name=="PUFLSZN")
        {
            ...
            for(size_t stage=0; i!=n and stage!=STEPS_MAX_LOAD_RELAY_STAGE; ++i, ++stage)
            {
        ...
        for(size_t stage=0; stage!=N; ++stage)
        {
            ...
            if(is_discrete_stage_timer_started(stage))
            {
                if(is_discrete_stage_timer_timed_out(stage) and is_minimum_frequency_declining())
                {
                    ...
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "PUFLS discrete stage %zu timer of %s is timed out at time %.3fs.\n"
            ...
            {
                char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "PUFLS discrete stage %zu timer of %s is started at time %.3fs.\n"
                        "%f%% loads are tripped.",stage,get_compound_device_name().c_str(),TIME,
                        get_discrete_stage_shed_scale_in_pu(stage)*100.0);
                toolkit.show_information(buffer);
...
{
    double total = 0.0;
    for(size_t stage=0; stage!=n and stage!=STEPS_MAX_LOAD_RELAY_STAGE; ++stage)
    {
