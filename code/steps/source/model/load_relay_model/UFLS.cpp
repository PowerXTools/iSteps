...
UFLS::UFLS(iSTEPS& toolkit) : LOAD_FREQUENCY_RELAY_MODEL(toolkit),
...
void UFLS::clear()
{
    ...
    for(size_t i=0; i!=STEPS_MAX_LOAD_RELAY_STAGE; ++i)
    {
    ...
    if(mode==DYNAMIC_RELAY_MODE)
    {
        ...
        for(size_t i=0; i!=STEPS_MAX_LOAD_RELAY_STAGE; ++i)
        {
            ...
            if(is_stage_breaker_timer_started(i))
            {
                ...
                {
                    osstream<<"UFLS stage "<<i<<" timer of "<<get_compound_device_name()<<" is timed out at time "<<current_time<<" s."<<endl
            ...
            if(is_stage_delayer_timer_started(i)) // delayer started
            {
                ...
                {
                    osstream<<"UFLS stage "<<i<<" timer of "<<get_compound_device_name()<<" is sending tripping signal to breaker at time "<<current_time<<" s since stage delayer timer is timed out."<<endl
                ...
                else// delayer not timed out
                {
                    ...
                    {
                        // need to reset
                        osstream<<"UFLS stage "<<i<<" timer of "<<get_compound_device_name()<<" is reset at time "<<current_time<<" s due to recovery of frequency."<<endl
                          <<"Current frequency is "<<f<<" Hz, and stage frequency threshold is "<<get_frequency_threshold_in_Hz_of_stage(i)<<" Hz.";
                        toolkit.show_information(osstream);
            ...
            else// delayer not started. check if need to start
            {
                if(f<get_frequency_threshold_in_Hz_of_stage(i))
                {
                    osstream<<"UFLS stage "<<i<<" timer of "<<get_compound_device_name()<<" is started at time "<<current_time<<" s due to drop of frequency."<<endl
                      <<"Current frequency is "<<f<<" Hz, and stage frequency threshold is "<<get_frequency_threshold_in_Hz_of_stage(i)<<" Hz.";
                    toolkit.show_information(osstream);
...
void UFLS::trip_stage(size_t i)
{
    ...
    if(i<STEPS_MAX_LOAD_RELAY_STAGE)
    {
        if(not is_stage_tripped(i))
        {
            ...
            DYN_SIMULATOR& sim = toolkit.get_dynamic_simulator();
...
string UFLS::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
    LOAD* load = get_load_pointer();
...
void UFLS::prepare_model_data_table()
{
    ...
    for(size_t stage=0; stage<STEPS_MAX_LOAD_RELAY_STAGE; ++stage)
    {
