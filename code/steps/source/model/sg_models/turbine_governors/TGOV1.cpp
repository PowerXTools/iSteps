...
TGOV1::TGOV1(iSTEPS& toolkit) : TURBINE_GOVERNOR_MODEL(toolkit),
                               governor(toolkit),
                               turbine(toolkit),
                               damping(toolkit)
{
...
string TGOV1::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
void TGOV1::prepare_model_internal_variable_table()
{
    ...
    add_model_internal_variable_name_and_index_pair("STATE@TURBINE", i); i++;
}
...
void TGOV1::build_linearized_matrix_ABCD()
{
    return;
}
