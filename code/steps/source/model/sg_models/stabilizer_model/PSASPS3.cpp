...
#include <vector>
PSASPS3::PSASPS3(iSTEPS& toolkit) : STABILIZER_MODEL(toolkit),
...
void PSASPS3::run(DYNAMIC_MODE mode)
{
    ...
    for(size_t i=0; i<STEPS_MODEL_FEEDBACK_LOOP_INTEGRATION_COUNT; ++i)
    {
...
string PSASPS3::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
void PSASPS3::prepare_model_internal_variable_table()
{
    ...
    add_model_internal_variable_name_and_index_pair("SIGNAL@SLOT 3", i); i++;
    add_model_internal_variable_name_and_index_pair("STATE@DEDC BLOCK 1", i); i++;
    add_model_internal_variable_name_and_index_pair("STATE@DEDC BLOCK 2", i); i++;
    ...
    add_model_internal_variable_name_and_index_pair("STATE@PHASE TUNER 4", i); i++;
    add_model_internal_variable_name_and_index_pair("OUTPUT@DEDC BLOCK 1", i); i++;
    add_model_internal_variable_name_and_index_pair("OUTPUT@DEDC BLOCK 2", i); i++;
    ...
    add_model_internal_variable_name_and_index_pair("OUTPUT@PHASE TUNER 4", i); i++;
}
...
void PSASPS3::build_linearized_matrix_ABCD()
{
    return;
}
