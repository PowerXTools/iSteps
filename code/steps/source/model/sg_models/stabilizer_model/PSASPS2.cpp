...
#include <vector>
PSASPS2::PSASPS2(iSTEPS& toolkit) : STABILIZER_MODEL(toolkit),
                                   sensor_w(toolkit),
                                   sensor_p(toolkit),
                                   sensor_t(toolkit),
...
string PSASPS2::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
void PSASPS2::prepare_model_internal_variable_table()
{
    ...
    add_model_internal_variable_name_and_index_pair("STATE@PMECH SENSOR", i); i++;
    ...
    add_model_internal_variable_name_and_index_pair("OUTPUT@PMECH SENSOR", i); i++;
...
void PSASPS2::build_linearized_matrix_ABCD()
{
    return;
}
