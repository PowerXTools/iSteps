...
using namespace std;
PSASPS1::PSASPS1(iSTEPS& toolkit) : STABILIZER_MODEL(toolkit),
                                   dedc_block(toolkit),
...
string PSASPS1::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
void PSASPS1::prepare_model_internal_variable_table()
{
    ...
    add_model_internal_variable_name_and_index_pair("STATE@DEDC BLOCK", i); i++;
    ...
    add_model_internal_variable_name_and_index_pair("OUTPUT@DEDC BLOCK", i); i++;
...
void PSASPS1::build_linearized_matrix_ABCD()
{
    return;
}
