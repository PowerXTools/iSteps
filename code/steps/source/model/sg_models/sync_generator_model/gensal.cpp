...
GENSAL::GENSAL(iSTEPS& toolkit) : SYNC_GENERATOR_MODEL(toolkit)
{
    clear();
}
...
GENSAL::GENSAL(const GENSAL& model):SYNC_GENERATOR_MODEL(model.get_toolkit())
{
...
void GENSAL::run(DYNAMIC_MODE mode)
{
    ...
    // q-axis
    input = saturation*Flux_dq.imag()*(xq-xl)/(xd-xl)+Idq.imag()*(xq-xpp);
...
string GENSAL::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
