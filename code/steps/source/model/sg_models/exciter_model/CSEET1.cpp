...
using namespace std;
CSEET1::CSEET1(iSTEPS& toolkit) : EXCITER_MODEL(toolkit),
                                 sensor(toolkit),
                                 serial_tuner1_lead_lag(toolkit),
                                 serial_tuner1_pi(toolkit),
                                 serial_tuner2(toolkit),
                                 parallel_tuner_integral(toolkit),
                                 parallel_tuner_differential(toolkit),
                                 regulator1(toolkit),
                                 regulator2(toolkit),
                                 feedbacker(toolkit),
                                 exciter(toolkit)
...
CSEET1::CSEET1(const CSEET1& model) : EXCITER_MODEL(model.get_toolkit()),
                                      ...
                                      parallel_tuner_integral(model.get_toolkit()),
                                      parallel_tuner_differential(model.get_toolkit()),
                                      regulator1(model.get_toolkit()),
                                      regulator2(model.get_toolkit()),
...
void CSEET1::run(DYNAMIC_MODE mode)
{
    ...
    for(size_t i=0; i<STEPS_MODEL_FEEDBACK_LOOP_INTEGRATION_COUNT; ++i)
    {
        ...
        double input = Vref+Vs-sensor.get_output();
...
double CSEET1::get_excitation_voltage_in_pu()
{
    ...
    double Fex = get_Fex(Ve, gen_model->get_field_current_in_pu_based_on_mbase());
    double Efd = Ve*Fex;
...
void CSEET1::check()
{
    ...
    if(vemax<=0.0)
    {
        osstream<<"Vemax<=0.0 was detected: Vemax="<<vemax<<"\n";
        error_found = true;
    }
    double efdmax = get_Efdmax_in_pu();
    if(efdmax<=0.0)
...
string CSEET1::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
double CSEET1::get_minimum_nonzero_time_constant_in_s()
{
    ...
    if(get_TF_in_s()!=0.0 and mint>get_TF_in_s())
        mint = get_TF_in_s();
    if(get_T5_in_s()!=0.0 and mint>get_T5_in_s())
        mint = get_T5_in_s();
...
void CSEET1::build_linearized_matrix_ABCD()
{
    return;
}
