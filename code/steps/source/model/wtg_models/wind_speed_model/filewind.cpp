...
using namespace std;
FILEWIND::FILEWIND(iSTEPS& toolkit) : WIND_SPEED_MODEL(toolkit)
{
    clear();
}
...
{
    for(size_t i=0; i<STEPS_MAX_WIND_SPEED_RECORD_SIZE; ++i)
    {
...
size_t FILEWIND::get_wind_record_count() const
{
    size_t n = 0;
    for(size_t i=0; i<STEPS_MAX_WIND_SPEED_RECORD_SIZE; ++i)
    {
...
{
    size_t n = get_wind_record_count();
    if(n>=1)
    {
        if(pos<=n-1)
...
void FILEWIND::prepare_model_internal_variable_table()
{
    ...
    size_t i=0;
    add_model_internal_variable_name_and_index_pair("WIND SPEED IN M/S", i); i++;
}
