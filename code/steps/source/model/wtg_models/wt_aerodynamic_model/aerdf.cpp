...
AERDF::AERDF(iSTEPS& toolkit) : WT_AERODYNAMIC_MODEL(toolkit)
{
...
string AERDF::get_standard_psse_string(bool export_internal_bus_number) const
{
    ostringstream osstream;
...
//    iSTEPS& toolkit = get_toolkit();
    ...
    size_t Npitch = (*pitch_angles).size();
    ...
    // export Cp
    size_t Nlambda = (*tip_speed_ratios).size();
    for(size_t i=0; i<Nlambda; ++i)
    {
        fid<<(*tip_speed_ratios)[i];
        for(size_t j=0; j<Npitch; ++j)
            fid<<", "<<(*Cp_Matrix)[i][j];
        fid<<endl;
    }
