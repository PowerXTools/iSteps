...
VSCHVDCP0::VSCHVDCP0(iSTEPS& toolkit):VSC_HVDC_NETWORK_MODEL(toolkit)
{
}
...
void VSCHVDCP0::initialize_converter_and_nonconverter_physical_internal_bus_pair()
{
    ...
    size_t n_dcbus = vsc_hvdc->get_dc_bus_count();
    ...
    if(n_dcbus!=0)
    {
        ...
        size_t count_converter = INDEX_NOT_EXIST, count_nonconverter = INDEX_NOT_EXIST;
        for(size_t i=0; i!=n_dcbus; ++i)
        {
            size_t bus_number = inphno_all_buses.get_physical_bus_number_of_internal_bus_number(i);
            size_t converter_index = vsc_hvdc->get_converter_index_with_dc_bus(bus_number);
...
{
    VSC_HVDC* vsc_hvdc = get_vsc_hvdc_pointer();
    size_t n = vsc_hvdc->get_dc_bus_count();
...
{
    VSC_HVDC* vsc_hvdc = get_vsc_hvdc_pointer();
    size_t n = vsc_hvdc->get_dc_line_count();
    for(size_t i=0; i!=n; ++i)
    {
        if(vsc_hvdc->get_dc_line_status(i)==true)
        {
            size_t ibus = vsc_hvdc->get_dc_line_sending_side_bus(i);
            size_t jbus = vsc_hvdc->get_dc_line_receiving_side_bus(i);
            double g = 1.0/vsc_hvdc->get_dc_line_resistance_in_ohm(i);
            size_t i_inbus = inphno_all_buses.get_internal_bus_number_of_physical_bus_number(ibus);
            size_t j_inbus = inphno_all_buses.get_internal_bus_number_of_physical_bus_number(jbus);
...
void VSCHVDCP0::split_dynamic_dc_network_matrix_to_4_sub_matrix()
{
    ...
    for(int k=0; k!=n; ++k)
    {
        ...
        size_t ibus = inphno_all_buses.get_physical_bus_number_of_internal_bus_number(i);
        size_t jbus = inphno_all_buses.get_physical_bus_number_of_internal_bus_number(j);
...
void VSCHVDCP0::solve_dynamic_network_with_quasi_steady_state_model()
{
    ...
    */
    ...
    size_t m = vsc_hvdc->get_dc_bus_count();
    size_t n_nonconverter_bus = m - n;
    ...
    for(size_t i=0; i!=n; ++i)
    {
        size_t dcbus_index = vsc_hvdc->get_dc_bus_index_with_converter_index(i);
        size_t dcbus = vsc_hvdc->get_dc_bus_number(dcbus_index);
    ...
    if(n_nonconverter_bus!=0)
    {
        ...
        for(size_t i=0; i!=n_nonconverter_bus; ++i)
        ...
        while(true)
        {
            for(size_t i=0; i!=n_nonconverter_bus; ++i)
            {
                size_t dcbus = inphno_nonconverter_buses.get_physical_bus_number_of_internal_bus_number(i);
                size_t index = vsc_hvdc->dc_bus_no2index(dcbus);
        ...
        for(size_t i=0; i!=n; ++i)
        {
            ...
            size_t index = vsc_hvdc->get_converter_index_with_dc_bus(dcbus);
            IC[i] += IC_N[i];
