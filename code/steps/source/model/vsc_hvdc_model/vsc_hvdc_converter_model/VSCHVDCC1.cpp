...
#include <iostream>
using namespace std;
VSCHVDCC1::VSCHVDCC1(iSTEPS& toolkit):VSC_HVDC_CONVERTER_MODEL(toolkit),
                                    udc_block(toolkit),
...
void VSCHVDCC1::copy_from_const_model(const VSCHVDCC1& model)
{
    ...
    set_dc_voltage_response_block_Kp(model.get_dc_voltage_response_block_Kp());
    set_dc_voltage_response_block_Ki(model.get_dc_voltage_response_block_Ki());
...
bool VSCHVDCC1::setup_model_with_steps_string_vector(vector<string>& data)
{
    ...
    if(data.size()>=22)
    {
        ...
        if(model_name==get_model_name())
        {
            ...
            Kp = get_double_data(data[data_index],"0.0"); data_index++;
            set_dc_voltage_response_block_Kp(Kp);

            Ki = get_double_data(data[data_index],"0.0"); data_index++;
            set_dc_voltage_response_block_Ki(Ki);
...
//    double time = toolkit.get_dynamic_simulation_time_in_s();
    ...
    if(fabs(input)<0.001)
        input=0;
    input /= get_converter_dc_base_voltage_in_kV();
    dc_voltage_response_block.set_input(input);
    dc_voltage_response_block.run(mode);
...
string VSCHVDCC1::get_standard_psse_string(bool export_internal_bus_number) const
{
    ...
    osstream<<setw(8)<<vsc_name<<", "
            ...
            <<setw(8)<<setprecision(3)<<get_dc_voltage_response_block_Kp()<<", "
            <<setw(8)<<setprecision(3)<<get_dc_voltage_response_block_Ki()<<"/ ";
