...
LCC_STATION& LCC_STATION::operator=(const LCC_STATION& station)
{
    ...
    for(size_t i=0; i<get_number_of_converters(); ++i)
        converters[i] = *(station.get_converter(i));
    return *this;
}

void LCC_STATION::set_number_of_converters(size_t n)
{
    clear();
    nconverters = n;
    converters = (LCC*) malloc(sizeof(LCC)*nconverters);
    for(size_t i = 0; i < nconverters; ++i)
    {
        converters[i].clear();
...
void LCC_STATION::set_converter(size_t converter_index, LCC& converter)
{
    if(converter_index < nconverters)
        converters[converter_index] = converter;
}

size_t LCC_STATION::get_number_of_converters() const
{
    return nconverters;
}

const LCC* LCC_STATION::get_converter(size_t converter_index) const
{
    if(converter_index < nconverters)
    ...
    else
    {
        ...
        get_toolkit().show_information(osstream);

        return nullptr;
    }
}

LCC* LCC_STATION::get_mutable_converter(size_t converter_index)
{
    if(converter_index < nconverters)
        return converters+converter_index;
    else
...
size_t LCC_STATION::get_total_power_percent_of_converter_station() const
{
    double percent = 0.0;
    for(size_t i = 0; i < nconverters; ++i)
...
bool LCC_STATION::is_connected_to_bus(size_t bus) const
{
    size_t n = get_number_of_converters();
...
bool LCC_STATION::is_in_area(size_t area) const
{
    size_t n = get_number_of_converters();
    for(unsigned i = 0; i<n; ++i)
...
bool LCC_STATION::is_in_zone(size_t zone) const
{
    size_t n = get_number_of_converters();
    for(unsigned i = 0; i<n; ++i)
