...
using namespace std;
AC_LINE::AC_LINE(iSTEPS& toolkit) : NONBUS_DEVICE(toolkit)
{
    clear();
}
...
void AC_LINE::set_sending_side_bus(size_t bus)
{
...
void AC_LINE::set_receiving_side_bus(size_t bus)
{
    ostringstream osstream;
    iSTEPS& toolkit = get_toolkit();
    if(bus!=0)
...
void AC_LINE::set_meter_end_bus(size_t meter_bus)
{
...
size_t AC_LINE::get_sending_side_bus() const
{
    return sending_side_bus;
}

size_t AC_LINE::get_receiving_side_bus() const
{
...
size_t AC_LINE::get_identifier_index() const
{
    return identifier_index;
}

size_t AC_LINE::get_name_index() const
{
    return name_index;
}
...
size_t AC_LINE::get_meter_end_bus() const
{
...
void AC_LINE::set_fault(size_t to_bus, double location, const FAULT& fault)
{
...
double AC_LINE::get_fault_location_of_fault(size_t index) const
{
...
FAULT AC_LINE::get_fault_at_location(size_t to_bus, double location) const
{
    FAULT nofault;
...
void AC_LINE::clear_fault_at_location(size_t to_bus, double location)
{
    if(is_connected_to_bus(to_bus) and (location>=0.0 and location<=1.0))
    {
        iSTEPS& toolkit = get_toolkit();
...
size_t AC_LINE::get_fault_count() const
{
    return faults.size();
}
...
bool AC_LINE::is_connected_to_bus(size_t bus) const
{
    if(get_sending_side_bus()==bus or get_receiving_side_bus()==bus) return true;
    else                                                             return false;
}

bool AC_LINE::is_in_area(size_t area) const
{
    iSTEPS& toolkit = get_toolkit();
    PF_DATA& psdb = toolkit.get_database();
...
bool AC_LINE::is_in_zone(size_t zone) const
{
    iSTEPS& toolkit = get_toolkit();
    PF_DATA& psdb = toolkit.get_database();
    BUS* busptr_send = psdb.get_bus(get_sending_side_bus());
...
MODEL* AC_LINE::get_model_of_type(string model_type, size_t index)
{
...
//            complex<double> Yr = 0.5*Y + get_shunt_zero_sequence_y_at_receiving_side_in_pu();
            ...
            for(size_t i=0; i<Y_mutual.size(); i++)
            {
