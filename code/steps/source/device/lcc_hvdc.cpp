...
LCC_HVDC::LCC_HVDC(iSTEPS& toolkit) : NONBUS_DEVICE(toolkit)
{
...
void LCC_HVDC::set_converter_count(CONVERTER_SIDE side, const size_t n)
{
    station[side].set_number_of_converters(n);
}

const LCC* LCC_HVDC::get_immutable_converter(CONVERTER_SIDE side, const size_t converter_index) const
{
    return station[side].get_converter(converter_index);
}

LCC* LCC_HVDC::get_mutable_converter(CONVERTER_SIDE side, const size_t converter_index)
{
    return station[side].get_mutable_converter(converter_index);
}

void LCC_HVDC::set_converter_bus(CONVERTER_SIDE side, const size_t converter_index, const size_t bus)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_bus(bus);
}

void LCC_HVDC::set_converter_valve_side_bus_name(CONVERTER_SIDE side, const size_t converter_index, string name)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_valve_side_bus_name(name);
}

void LCC_HVDC::set_converter_power_percent(CONVERTER_SIDE side, const size_t converter_index, double percent)
{
    LCC* converter = get_mutable_converter(side, converter_index);
...
void LCC_HVDC::set_converter_number_of_bridge(CONVERTER_SIDE side, const size_t converter_index, const size_t nbridge)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_number_of_bridge(nbridge);
}

void LCC_HVDC::set_converter_voltage_drop_per_bridge_in_kV(CONVERTER_SIDE side, const size_t converter_index, const double V)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_voltage_drop_per_bridge_in_kV(V);
}

void LCC_HVDC::set_converter_max_alpha_or_gamma_in_deg(CONVERTER_SIDE side, const size_t converter_index, const double angle)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_max_angle_in_deg(angle);
}

void LCC_HVDC::set_converter_min_alpha_or_gamma_in_deg(CONVERTER_SIDE side, const size_t converter_index, const double angle)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_min_angle_in_deg(angle);
}

void LCC_HVDC::set_converter_transformer_grid_side_base_voltage_in_kV(CONVERTER_SIDE side, const size_t converter_index, const double V)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_grid_side_base_voltage_in_kV(V);
}

void LCC_HVDC::set_converter_transformer_valve_side_base_voltage_in_kV(CONVERTER_SIDE side, const size_t converter_index, const double V)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_valve_side_base_voltage_in_kV(V);
}

void LCC_HVDC::set_converter_transformer_impedance_in_ohm(CONVERTER_SIDE side, const size_t converter_index, const complex<double> z)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_impedance_in_ohm(z);
}

void LCC_HVDC::set_converter_transformer_admittance_in_siemens(CONVERTER_SIDE side, const size_t converter_index, const complex<double> y)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_admittance_in_siemens(y);
}

void LCC_HVDC::set_converter_transformer_max_tap_in_pu(CONVERTER_SIDE side, const size_t converter_index, const double maxtap)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_max_tap_in_pu(maxtap);
}

void LCC_HVDC::set_converter_transformer_min_tap_in_pu(CONVERTER_SIDE side, const size_t converter_index, const double mintap)
{
    LCC* converter = get_mutable_converter(side, converter_index);
    if(converter!=nullptr) converter->set_transformer_min_tap_in_pu(mintap);
}

void LCC_HVDC::set_converter_transformer_number_of_taps(CONVERTER_SIDE side, const size_t converter_index, const size_t ntap)
{
    LCC* converter = get_mutable_converter(side, converter_index);
...
size_t LCC_HVDC::get_name_index() const
{
    return name_index;
}

size_t LCC_HVDC::get_converter_count(CONVERTER_SIDE side) const
{
    return station[side].get_number_of_converters();
}

size_t LCC_HVDC::get_converter_bus(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
    return converter->get_bus();
}

/*BUS* LCC_HVDC::get_converter_bus_pointer(CONVERTER_SIDE side, size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
string LCC_HVDC::get_converter_valve_side_bus_name(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
    return converter->get_valve_side_bus_name();
}

double LCC_HVDC::get_converter_power_percent(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
size_t LCC_HVDC::get_converter_number_of_bridge(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_voltage_drop_per_bridge_in_kV(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_max_alpha_or_gamma_in_deg(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_min_alpha_or_gamma_in_deg(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_cos_max_alpha_or_gamma(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_cos_min_alpha_or_gamma(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_transformer_grid_side_base_voltage_in_kV(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_transformer_valve_side_base_voltage_in_kV(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
complex<double> LCC_HVDC::get_converter_transformer_impedance_in_ohm(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
complex<double> LCC_HVDC::get_converter_transformer_admittance_in_siemens(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_transformer_max_tap_in_pu(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
double LCC_HVDC::get_converter_transformer_min_tap_in_pu(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
size_t LCC_HVDC::get_converter_transformer_number_of_taps(CONVERTER_SIDE side, const size_t converter_index) const
{
    const LCC* converter = get_immutable_converter(side, converter_index);
...
bool LCC_HVDC::is_connected_to_bus(size_t bus) const
{
    for(size_t converter=0; converter<2; ++converter)
    {
...
bool LCC_HVDC::is_in_area(size_t area) const
{
    for(size_t converter=0; converter<2; ++converter)
    {
        CONVERTER_SIDE side = RECTIFIER;
...
bool LCC_HVDC::is_in_zone(size_t zone) const
{
    for(size_t converter=0; converter<2; ++converter)
    {
        CONVERTER_SIDE side = RECTIFIER;
...
DEVICE_ID LCC_HVDC::get_device_id() const
{
    ...
    TERMINAL terminal;
    size_t n = get_converter_count(RECTIFIER);
    for(size_t converter_index=0; converter_index<n; ++converter_index)
...
void LCC_HVDC::show_solved_steady_state() const
{
    ...
    for(size_t converter=0; converter<2; ++converter)
    {
        ...
        size_t n = get_converter_count(side);
        for(size_t converter_index=0; converter_index<n; ++converter_index)
        {
            const LCC* converter = get_immutable_converter(side, converter_index);
...
MODEL* LCC_HVDC::get_model_of_type(string model_type, size_t index)
{
    return nullptr;
}
