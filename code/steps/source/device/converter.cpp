...
CONVERTER::CONVERTER(iSTEPS& toolkit)
{
    clear();
    set_toolkit(toolkit);
}
...
void CONVERTER::set_toolkit(iSTEPS& toolkit) { this->toolkit = (&toolkit); }
iSTEPS& CONVERTER::get_toolkit() const { return *toolkit; }
...
void CONVERTER::set_bus(const size_t bus)
{
...
void CONVERTER::set_transformer_number_of_taps(const size_t n) { transformer_number_of_taps = n; try_to_set_transformer_tap_step(); try_to_set_transformer_tap_fixed_flag(); }
...
void CONVERTER::try_to_set_transformer_tap_fixed_flag()
{
    ...
    size_t nTap = get_transformer_number_of_taps();
...
size_t CONVERTER::get_bus() const { return bus; }
...
size_t CONVERTER::get_transformer_number_of_taps() const { return transformer_number_of_taps; }
...
bool CONVERTER::is_connected_to_bus(size_t bus) const {return get_bus() == bus;}
bool CONVERTER::is_in_area(size_t area) const {return get_bus_pointer()->is_in_area(area); }
bool CONVERTER::is_in_zone(size_t zone) const {return get_bus_pointer()->is_in_zone(zone); }
