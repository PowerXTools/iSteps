...
size_t SPARSE_MATRIX_CSPARSE::get_memory_usage()
{
    size_t n = matrix_real->nzmax*(sizeof(csi)+sizeof(double))+
...
SPARSE_MATRIX_CSPARSE inv(SPARSE_MATRIX_CSPARSE&A)
{
    ...
    size_t n = A.get_matrix_column_count();
...
SPARSE_MATRIX_CSPARSE concatenate_matrix_diagnally(const vector<SPARSE_MATRIX_CSPARSE*> matrix)
{
    ...
    for(size_t i = 0; i<N; ++i)
    {
        ...
        if(imatrix->matrix_in_triplet_form())
        {
            ...
            default_toolkit.show_information(osstream);
            break;
        }
        size_t nz = imatrix->get_matrix_entry_count();
        if(nz == 0) continue;

        for(size_t j=0; j<nz; ++j)
        {
            double value = imatrix->get_real_entry_value(j);
            size_t mi = imatrix->get_row_number_of_entry_index(j);
            size_t ni = imatrix->get_column_number_of_entry_index(j);
...
SPARSE_MATRIX_CSPARSE build_identity_matrix(size_t n)
{
    vector<double> b;
    for(size_t i=0; i<n; ++i)
        b.push_back(0.0);
...
SPARSE_MATRIX_CSPARSE change_matrix_to_new_size(const SPARSE_MATRIX_CSPARSE&A, size_t mrow, size_t ncol)
{
    size_t m = A.get_matrix_row_count();
    size_t n = A.get_matrix_column_count();
...
SPARSE_MATRIX_CSPARSE expand_matrix_to_new_size(const SPARSE_MATRIX_CSPARSE&A, size_t mrow, size_t ncol)
{
    size_t m = A.get_matrix_row_count();
    size_t n = A.get_matrix_column_count();
    if(mrow<m or ncol<n)
...
SPARSE_MATRIX_CSPARSE shrink_matrix_to_new_size(const SPARSE_MATRIX_CSPARSE&A, size_t mrow, size_t ncol)
{
    size_t m = A.get_matrix_row_count();
    size_t n = A.get_matrix_column_count();
    if(mrow>m or ncol>n)
    ...
    else
    {
        static SPARSE_MATRIX_CSPARSE B;
        B.clear();
        size_t nz = A.get_matrix_entry_count();
        for(size_t k=0; k<nz; ++k)
        {
            size_t i = A.get_row_number_of_entry_index(k);
            size_t j = A.get_column_number_of_entry_index(k);
            if(i>=mrow or j>=ncol)
                continue;
            else
            {
