...
void DEVICE_ID::set_device_identifier_index(size_t index){
    if(allow_identifier)
    {
...
void DEVICE_ID::set_device_name_index(size_t index)
{
    if(allow_name)
    {
...
size_t DEVICE_ID::get_device_identifier_index() const
{
    return device_identifier_index;
}

size_t DEVICE_ID::get_device_name_index() const
{
...
string DEVICE_ID::get_compound_device_name() const
{
    ...
    if(is_valid())
    {
        ...
        if(device_type==STEPS_VSC_HVDC)
        {
            ...
            for(size_t i=0; i<bus_count-1; ++i)
