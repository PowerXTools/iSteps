...
void CONTINUOUS_BUFFER::copy_from_constant_buffer(const CONTINUOUS_BUFFER& buffer)
{
    ...
    for(size_t i=0; i<STEPS_MAX_CONTINUOUS_BUFFER_SIZE; ++i)
    {
...
void CONTINUOUS_BUFFER::set_buffer_size(size_t n)
{
    if(n>STEPS_MAX_CONTINUOUS_BUFFER_SIZE)
    {
        iSTEPS& toolkit = get_toolkit(__PRETTY_FUNCTION__);
...
size_t CONTINUOUS_BUFFER::get_index_of_buffer_tail() const
{
...
void CONTINUOUS_BUFFER::update_buffer_value_at_delay_index(size_t index, double value)
{
    index = get_storage_index_of_delay_index(index);
...
{
    size_t nA = A.get_buffer_size();
    size_t nB = B.get_buffer_size();
    if(nA==nB)
    {
        for(size_t index = nA; index!=0; --index)
        {
