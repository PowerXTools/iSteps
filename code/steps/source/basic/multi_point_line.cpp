...
void MULTI_POINT_LINE::copy_from_const_line(const MULTI_POINT_LINE& m)
{
    ...
    for(size_t i=0; i<STEPS_MAX_MULTI_POINT_LINE_SIZE; ++i)
    {
...
size_t MULTI_POINT_LINE::get_valid_point_size() const
{
    size_t valid_point_count = STEPS_MAX_MULTI_POINT_LINE_SIZE;
    for(size_t i=0; i<STEPS_MAX_MULTI_POINT_LINE_SIZE; ++i)
    {
...
double MULTI_POINT_LINE::get_y_at_x(double x)
{
    ...
    size_t i_less = get_the_last_point_with_x_less_than_or_equal_to(x);
...
size_t MULTI_POINT_LINE::get_the_last_point_with_x_less_than_or_equal_to(double x)
{
    size_t n = get_valid_point_size();
...
double MULTI_POINT_LINE::get_point_x_with_index(size_t i) const
{
...
double MULTI_POINT_LINE::get_point_y_with_index(size_t i) const
{
    if(i<STEPS_MAX_MULTI_POINT_LINE_SIZE)
        return line[i][1];
    else
...
void MULTI_POINT_LINE::set_point_x_with_index(size_t i, double value)
{
    if(i<STEPS_MAX_MULTI_POINT_LINE_SIZE)
        line[i][0] = value;
}

void MULTI_POINT_LINE::set_point_y_with_index(size_t i, double value)
{
    if(i<STEPS_MAX_MULTI_POINT_LINE_SIZE)
        line[i][1] = value;
}
