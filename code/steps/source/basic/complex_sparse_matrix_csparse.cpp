...
void COMPLEX_SPARSE_MATRIX_CSPARSE::clear()
{
    ...
    matrix_complex = cxs_spalloc(1,1,1,1,1);
...
void COMPLEX_SPARSE_MATRIX_CSPARSE::convert_to_triplet_form()
{
    if(matrix_in_compressed_column_form())
    {
        ...
        tempi = matrix_complex->p;
        matrix_complex->p = mat_complex->p;
        mat_complex->p = tempi;
        tempi = matrix_complex->i;
        matrix_complex->i = mat_complex->i;
        mat_complex->i = tempi;
        tempd = matrix_complex->x;
        matrix_complex->x = mat_complex->x;
        mat_complex->x = tempd;
        // free temp mat
...
void COMPLEX_SPARSE_MATRIX_CSPARSE::compress_and_merge_duplicate_entries()
{
    ...
    if(matrix_in_triplet_form())
    {
        ...
        tempi = matrix_complex->p;
        matrix_complex->p = mat->p;
        mat->p = tempi;
        tempi = matrix_complex->i;
        matrix_complex->i = mat->i;
        mat->i = tempi;
        tempd = matrix_complex->x;
        matrix_complex->x = mat->x;
        mat->x = tempd;
        // free temp mat
        cxs_spfree(mat);
...
vector<size_t> COMPLEX_SPARSE_MATRIX_CSPARSE::get_reorder_permutation()
{
...
size_t COMPLEX_SPARSE_MATRIX_CSPARSE::get_memory_usage()
{
    size_t n = matrix_complex->nzmax*(sizeof(int)+sizeof(complex<double>))+
