...
NET_MATRIX::NET_MATRIX(iSTEPS& toolkit)
{
...
iSTEPS& NET_MATRIX::get_toolkit() const
{
   return *toolkit;
}

void NET_MATRIX::clear_all_data()
{
...
void NET_MATRIX::set_generator_reactance_option(GENERATOR_REACTANCE_OPTION option)
{
   gen_X_option = option;
}

GENERATOR_REACTANCE_OPTION NET_MATRIX::get_generator_reactance_option()
{
   return gen_X_option;
}

void NET_MATRIX::set_consider_load_logic(bool logic)
{
   consider_load = logic;
}
bool NET_MATRIX::get_consider_load_logic()
{
   return consider_load;
}
void NET_MATRIX::set_consider_motor_load_logic(bool logic)
{
   consider_motor_load = logic;
}
bool NET_MATRIX::get_consider_motor_load_logic()
{
   return consider_motor_load;
}

void NET_MATRIX::set_option_of_dc_lines(DC_LINES_OPTION option)
{
   option_of_all_ordinary_dc_lines = option;
}

DC_LINES_OPTION NET_MATRIX::get_option_of_dc_lines()
{
   return option_of_all_ordinary_dc_lines;
}

void NET_MATRIX::build_initial_zero_matrix(STEPS_COMPLEX_SPARSE_MATRIX& matrix)
{
   ...
   for (size_t i = 0; i != nbus; ++i) {
      ...
      if (bus->get_bus_type() != OUT_OF_SERVICE) {
         size_t inbus = inphno.get_internal_bus_number_of_physical_bus_number(bus->get_bus_number());
         matrix.add_entry(inbus, inbus, 0);
      }
   }
}

void NET_MATRIX::build_initial_zero_matrix(STEPS_SPARSE_MATRIX& matrix)
{
   PF_DATA& psdb = toolkit->get_database();
   vector<BUS*> buses = psdb.get_all_buses();
   size_t nbus = buses.size();
...
void NET_MATRIX::build_network_Y_matrix()
{
   if (inphno.empty()) {
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_network_Y_matrix()
{
...
void NET_MATRIX::build_decoupled_network_B_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_SPARSE_MATRIX& NET_MATRIX::get_decoupled_network_BP_matrix()
{
...
STEPS_SPARSE_MATRIX& NET_MATRIX::get_decoupled_network_BQ_matrix()
{
   if (network_BP_matrix.matrix_in_triplet_form()) {
      build_decoupled_network_B_matrix();
   }
...
void NET_MATRIX::build_dc_network_B_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_SPARSE_MATRIX& NET_MATRIX::get_dc_network_B_matrix()
{
...
void NET_MATRIX::build_dynamic_network_Y_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_dynamic_network_Y_matrix()
{
   if (network_Y1_matrix.matrix_in_triplet_form()) {
...
void NET_MATRIX::build_sequence_network_Y_matrix()
{
...
void NET_MATRIX::build_positive_sequence_network_Y_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_positive_sequence_network_Y_matrix()
{
   if (network_Y1_matrix.matrix_in_triplet_form()) {
      build_positive_sequence_network_Y_matrix();
   }
...
void NET_MATRIX::build_negative_sequence_network_Y_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_negative_sequence_network_Y_matrix()
{
...
void NET_MATRIX::build_zero_sequence_network_Y_matrix()
{
   if (inphno.empty()) {
      initialize_physical_internal_bus_pair();
   }
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_zero_sequence_network_Y_matrix()
{
...
void NET_MATRIX::build_network_Z_matrix_from_this_Y_matrix()
{
...
void NET_MATRIX::build_sequence_network_Z_matrix()
{
...
void NET_MATRIX::build_positive_sequence_network_Z_matrix()
{
   set_this_Y_and_Z_matrix_as(network_Y1_matrix);
   build_network_Z_matrix_from_this_Y_matrix();
}

void NET_MATRIX::build_negative_sequence_network_Z_matrix()
{
   set_this_Y_and_Z_matrix_as(network_Y2_matrix);
   build_network_Z_matrix_from_this_Y_matrix();
}

void NET_MATRIX::build_zero_sequence_network_Z_matrix()
{
   set_this_Y_and_Z_matrix_as(network_Y0_matrix);
   build_network_Z_matrix_from_this_Y_matrix();
}

STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_positive_sequence_network_Z_matrix()
{
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_negative_sequence_network_Z_matrix()
{
...
STEPS_COMPLEX_SPARSE_MATRIX& NET_MATRIX::get_zero_sequence_network_Z_matrix()
{
...
vector<complex<double> > NET_MATRIX::get_positive_sequence_complex_impedance_of_column_with_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y1_matrix);
...
vector<complex<double> > NET_MATRIX::get_negative_sequence_complex_impedance_of_column_with_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y2_matrix);
   int bus_number = get_internal_bus_number_of_physical_bus(bus);
...
vector<complex<double> > NET_MATRIX::get_zero_sequence_complex_impedance_of_column_with_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y0_matrix);
   int bus_number = get_internal_bus_number_of_physical_bus(bus);
...
complex<double> NET_MATRIX::get_positive_sequence_self_admittance_of_physical_bus(size_t bus)
{
   int b = get_internal_bus_number_of_physical_bus(bus);
   return network_Y1_matrix.get_entry_value(b, b);
}

complex<double> NET_MATRIX::get_positive_sequence_mutual_admittance_between_physical_bus(size_t ibus, size_t jbus)
{
...
void NET_MATRIX::set_this_Y_and_Z_matrix_as(STEPS_COMPLEX_SPARSE_MATRIX& matrix)
{
...
void NET_MATRIX::build_this_jacobian_for_getting_impedance_from_this_Y_matrix()
{
   this_jacobian.clear();
   int n = this_Y_matrix_pointer->get_matrix_size();
   for (int j = 0; j < n; ++j) {
...
vector<double> NET_MATRIX::get_impedance_of_column_from_this_Y_matrix(size_t col)
{
   build_this_jacobian_for_getting_impedance_from_this_Y_matrix();
...
vector<complex<double> > NET_MATRIX::get_complex_impedance_of_column_from_this_Y_matrix(size_t col)
{
   build_this_jacobian_for_getting_impedance_from_this_Y_matrix();
...
complex<double> NET_MATRIX::get_self_impedance_of_physical_bus_from_this_Y_matrix(size_t bus)
{
   int bus_number = get_internal_bus_number_of_physical_bus(bus);
...
complex<double> NET_MATRIX::get_mutual_impedance_between_physical_bus_from_this_Y_matrix(size_t ibus, size_t jbus)
{
...
complex<double> NET_MATRIX::get_positive_sequence_self_impedance_of_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y1_matrix);
   return get_self_impedance_of_physical_bus_from_this_Y_matrix(bus);
}

complex<double> NET_MATRIX::get_positive_sequence_mutual_impedance_between_physical_bus(size_t ibus, size_t jbus)
{
   set_this_Y_and_Z_matrix_as(network_Y1_matrix);
...
complex<double> NET_MATRIX::get_negative_sequence_self_admittance_of_physical_bus(size_t bus)
{
   int b = get_internal_bus_number_of_physical_bus(bus);
   return network_Y2_matrix.get_entry_value(b, b);
}

complex<double> NET_MATRIX::get_negative_sequence_mutual_admittance_between_physical_bus(size_t ibus, size_t jbus)
{
   int ib = get_internal_bus_number_of_physical_bus(ibus);
   int jb = get_internal_bus_number_of_physical_bus(jbus);
...
complex<double> NET_MATRIX::get_negative_sequence_self_impedance_of_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y2_matrix);
   return get_self_impedance_of_physical_bus_from_this_Y_matrix(bus);
}

complex<double> NET_MATRIX::get_negative_sequence_mutual_impedance_between_physical_bus(size_t ibus, size_t jbus)
{
   set_this_Y_and_Z_matrix_as(network_Y2_matrix);
   return get_mutual_impedance_between_physical_bus_from_this_Y_matrix(ibus, jbus);
}

complex<double> NET_MATRIX::get_zero_sequence_self_admittance_of_physical_bus(size_t bus)
{
   int b = get_internal_bus_number_of_physical_bus(bus);
   return network_Y0_matrix.get_entry_value(b, b);
}

complex<double> NET_MATRIX::get_zero_sequence_mutual_admittance_between_physical_bus(size_t ibus, size_t jbus)
{
   set_this_Y_and_Z_matrix_as(network_Y0_matrix);
...
complex<double> NET_MATRIX::get_zero_sequence_self_impedance_of_physical_bus(size_t bus)
{
   set_this_Y_and_Z_matrix_as(network_Y0_matrix);
   return get_self_impedance_of_physical_bus_from_this_Y_matrix(bus);
}

complex<double> NET_MATRIX::get_zero_sequence_mutual_impedance_between_physical_bus(size_t ibus, size_t jbus)
{
   set_this_Y_and_Z_matrix_as(network_Y0_matrix);
   return get_mutual_impedance_between_physical_bus_from_this_Y_matrix(ibus, jbus);
}

void NET_MATRIX::add_ac_lines_to_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_ac_line_to_network(const AC_LINE& line)
{
   if (line.get_sending_side_breaker_status() == true or line.get_receiving_side_breaker_status() == true) {
      /*ostringstream osstream;
      osstream<<"Adding line '%s' connecting to bus %zu and %zu to network Y matrix.", (line.get_identifier()).c_str(),
...
void NET_MATRIX::add_transformers_to_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_transformer_to_network(const TRANSFORMER& trans)
{
...
void NET_MATRIX::add_two_winding_transformer_to_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or trans.get_winding_breaker_status(SECONDARY_SIDE) == true) {
      /*ostringstream osstream;
      osstream<<"Adding two-winding transformer '%s' connecting to bus %zu and %zu to network Y matrix.", (trans.get_identifier()).c_str(),
...
void NET_MATRIX::add_three_winding_transformer_to_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or
      ...
      /*ostringstream osstream;
      osstream<<"Adding three-winding transformer '%s' connecting to bus %zu, %zu, and %zu to network Y matrix.", (trans.get_identifier()).c_str(),
...
void NET_MATRIX::add_two_winding_transformer_to_network_v2(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or trans.get_winding_breaker_status(SECONDARY_SIDE) == true) {
      /*ostringstream osstream;
      osstream<<"Adding two-winding transformer '%s' connecting to bus %zu and %zu to network Y matrix.", (trans.get_identifier()).c_str(),
...
void NET_MATRIX::add_fixed_shunts_to_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_fixed_shunt_to_network(const FIXED_SHUNT& shunt)
{
   if (shunt.get_status() == true) {
      /*ostringstream osstream;
      osstream<<"Adding fixed shunt '%s' at bus %zu to network Y matrix.", (shunt.get_identifier()).c_str(),
                    shunt.get_shunt_bus());
      toolkit->show_information(osstream);
      */
...
void NET_MATRIX::add_ac_lines_to_decoupled_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<AC_LINE*> lines = psdb.get_all_ac_lines();
...
void NET_MATRIX::add_ac_line_to_decoupled_network(const AC_LINE& line)
{
   if (line.get_sending_side_breaker_status() == true or line.get_receiving_side_breaker_status() == true) {
      /*ostringstream osstream;
      osstream<<"Adding line '%s' connecting to bus %zu and %zu to network Y matrix.", (line.get_identifier()).c_str(),
...
void NET_MATRIX::add_transformers_to_decoupled_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<TRANSFORMER*> transformers = psdb.get_all_transformers();
...
void NET_MATRIX::add_transformer_to_decoupled_network(const TRANSFORMER& trans)
{
   if (trans.is_two_winding_transformer()) {
...
void NET_MATRIX::add_three_winding_transformer_to_decoupled_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or
         trans.get_winding_breaker_status(SECONDARY_SIDE) == true or
         trans.get_winding_breaker_status(TERTIARY_SIDE) == true) {
...
void NET_MATRIX::add_two_winding_transformer_to_decoupled_network_v2(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or trans.get_winding_breaker_status(SECONDARY_SIDE) == true) {
      /*ostringstream osstream;
      osstream<<"Adding two-winding transformer '%s' connecting to bus %zu and %zu to network Y matrix.", (trans.get_identifier()).c_str(),
...
void NET_MATRIX::add_fixed_shunts_to_decoupled_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<FIXED_SHUNT*> shunts = psdb.get_all_fixed_shunts();
...
void NET_MATRIX::add_fixed_shunt_to_decoupled_network(const FIXED_SHUNT& shunt)
{
   if (shunt.get_status() == true) {
      /*ostringstream osstream;
      osstream<<"Adding fixed shunt '%s' at bus %zu to network Y matrix.", (shunt.get_identifier()).c_str(),
...
void NET_MATRIX::add_ac_lines_to_dc_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<AC_LINE*> lines = psdb.get_all_ac_lines();
...
void NET_MATRIX::add_ac_line_to_dc_network(const AC_LINE& line)
{
   if (line.get_sending_side_breaker_status() == true or line.get_receiving_side_breaker_status() == true) {
      /*ostringstream osstream;
      osstream<<"Adding line '%s' connecting to bus %zu and %zu to network Y matrix.", (line.get_identifier()).c_str(),
...
void NET_MATRIX::add_transformers_to_dc_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<TRANSFORMER*> transformers = psdb.get_all_transformers();
...
void NET_MATRIX::add_transformer_to_dc_network(const TRANSFORMER& trans)
{
   if (trans.is_two_winding_transformer()) {
...
void NET_MATRIX::add_three_winding_transformer_to_dc_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or
         trans.get_winding_breaker_status(SECONDARY_SIDE) == true or
         trans.get_winding_breaker_status(TERTIARY_SIDE) == true) {
...
void NET_MATRIX::add_two_winding_transformer_to_dc_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or trans.get_winding_breaker_status(SECONDARY_SIDE) == true) {
      /*ostringstream osstream;
      osstream<<"Adding two-winding transformer '%s' connecting to bus %zu and %zu to network Y matrix.", (trans.get_identifier()).c_str(),
...
void NET_MATRIX::add_bus_fault_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<BUS*> buses = psdb.get_all_buses();
...
void NET_MATRIX::add_ac_lines_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<AC_LINE*> lines = psdb.get_all_ac_lines();
...
void NET_MATRIX::add_faulted_ac_line_to_dynamic_network(const AC_LINE& line)
{
   if (line.get_sending_side_breaker_status() == true or line.get_receiving_side_breaker_status() == true) {
      if (line.is_faulted()) {
         /*ostringstream osstream;
...
void NET_MATRIX::add_generators_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_generator_to_dynamic_network(const GENERATOR& gen)
{
...
void NET_MATRIX::add_wt_generators_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_wt_generator_to_dynamic_network(WT_GENERATOR& gen)
{
   if (gen.get_status() == true) {
...
void NET_MATRIX::add_pv_units_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_pv_unit_to_dynamic_network(PV_UNIT& pv_unit)
{
...
void NET_MATRIX::add_energy_storages_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_energy_storage_to_dynamic_network(ENERGY_STORAGE& es)
{
   if (es.get_status() == true) {
      ...
      if (esconmodel != NULL) {
         if (esconmodel->is_voltage_source()) {
            ...
            size_t bus = es.get_energy_storage_bus();
            size_t i = inphno.get_internal_bus_number_of_physical_bus_number(bus);
            this_Y_matrix_pointer->add_entry(i, i, 1.0 / Z);
         }
         //else // is current source
...
void NET_MATRIX::add_motor_loads_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_motor_load_to_dynamic_network(const LOAD& load)
{
   LOAD_MODEL* model = load.get_load_model();
   if (model != NULL) {
      if (model->is_voltage_source()) {
...
void NET_MATRIX::add_vsc_hvdcs_to_dynamic_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_vsc_hvdc_to_dynamic_network(const VSC_HVDC& vsc_hvdc)
{
         ...
         if (vsc_hvdc.get_converter_status(i) == true) {
            ...
            if (model != NULL) {
               if (model->is_voltage_source()) {
                  ...
                  size_t bus = vsc_hvdc.get_converter_ac_bus(i);
                  size_t i = inphno.get_internal_bus_number_of_physical_bus_number(bus);
                  this_Y_matrix_pointer->add_entry(i, i, y);
               }
            }
...
void NET_MATRIX::add_ac_lines_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<AC_LINE*> lines = psdb.get_all_ac_lines();
...
void NET_MATRIX::add_faulted_ac_line_to_positive_sequence_network(const AC_LINE& line)
{
...
void NET_MATRIX::add_transformers_to_positive_sequence_network()
{
   add_transformers_to_network();
}

void NET_MATRIX::add_loads_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<LOAD*> loads = psdb.get_all_loads();
...
void NET_MATRIX::add_load_to_positive_sequence_network(const LOAD& load)
{
...
void NET_MATRIX::add_sources_to_positive_sequence_network()
{
...
void NET_MATRIX::add_generators_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<GENERATOR*> generators = psdb.get_all_generators();
...
void NET_MATRIX::add_generator_to_positive_sequence_network(const GENERATOR& gen)
{
   ...
   if (gen.get_status() == true) {
      ...
      case SUBTRANSIENT_REACTANCE:
         X = gen.get_positive_sequence_subtransient_reactance_in_pu();
         break;
      case TRANSIENT_REACTANCE:
         X = gen.get_positive_sequence_transient_reactance_in_pu();
         break;
      case SYNCHRONOUS_REACTANCE:
         X = gen.get_positive_sequence_syncronous_reactance_in_pu();
         break;
      }
      double one_over_mbase = gen.get_one_over_mbase_in_one_over_MVA();
      double sbase = psdb.get_system_base_power_in_MVA();
...
void NET_MATRIX::add_wt_generators_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_wt_generator_to_positive_sequence_network(const WT_GENERATOR& wt_gen)
{
...
//    }
   ...
   if (wt_gen.get_status() == true) {
      ...
      case CONSTANT_SPEED_WT_GENERATOR:
         add_const_speed_wtg_to_positve_sequence_network(wt_gen);
         break;
      case DOUBLY_FED_WT_GENERATOR:
         add_doubly_fed_wtg_to_positive_sequence_network(wt_gen);
         break;
      case DIRECT_DRIVEN_WT_GENERATOR:
         add_direct_driven_wtg_to_positive_sequence_network(wt_gen);
         break;
      default:
         break;
      }
...
void NET_MATRIX::add_const_speed_wtg_to_positve_sequence_network(const WT_GENERATOR& wt_gen)
{
...
void NET_MATRIX::add_doubly_fed_wtg_to_positive_sequence_network(const WT_GENERATOR& wt_gen)
{
   //as voltage source
   if (wt_gen.get_status() == true) {
      PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_direct_driven_wtg_to_positive_sequence_network(const WT_GENERATOR& wt_gen)
{
   // as current source, so ignore its impedance
}

void NET_MATRIX::add_pv_units_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<PV_UNIT*> pv_units = psdb.get_all_pv_units();
...
void NET_MATRIX::add_pv_unit_to_positive_sequence_network(const PV_UNIT& pv_unit)
{
...
void NET_MATRIX::add_energy_storages_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<ENERGY_STORAGE*> eses = psdb.get_all_energy_storages();
...
void NET_MATRIX::add_energy_storage_to_positive_sequence_network(const ENERGY_STORAGE& es)
{
   return;
}

void NET_MATRIX::add_fixed_shunts_to_positive_sequence_network()
{
   add_fixed_shunts_to_network();
}

void NET_MATRIX::add_2t_lcc_hvdcs_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::add_2t_lcc_hvdc_to_positive_sequence_network(const LCC_HVDC2T& hvdc)
{
   PF_DATA& psdb = toolkit->get_database();
   double sbase = psdb.get_system_base_power_in_MVA();
   if (hvdc.get_status() == true) {
      ...
      size_t rectifier_bus = hvdc.get_converter_bus(RECTIFIER);
      size_t inverter_bus = hvdc.get_converter_bus(INVERTER);
      size_t r = inphno.get_internal_bus_number_of_physical_bus_number(rectifier_bus);
      size_t i = inphno.get_internal_bus_number_of_physical_bus_number(inverter_bus);
...
void NET_MATRIX::add_vsc_hvdcs_to_positive_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<VSC_HVDC*> vsc_hvdcs = psdb.get_all_vsc_hvdcs();
...
void NET_MATRIX::add_vsc_hvdc_to_positive_sequence_network(const VSC_HVDC& vsc_hvdc)
{
...
//    PF_DATA& psdb = toolkit->get_database();
   if (vsc_hvdc.get_status() == true) {
      size_t n = vsc_hvdc.get_converter_count();
      for (size_t i = 0; i < n; i++) {
         complex<double> Zc = vsc_hvdc.get_converter_commutating_impedance_in_pu_on_system_base(i);

         size_t ibus = vsc_hvdc.get_converter_ac_bus(i);
         size_t internal_ibus = inphno.get_internal_bus_number_of_physical_bus_number(ibus);
...
void NET_MATRIX::add_ac_lines_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<AC_LINE*> lines = psdb.get_all_ac_lines();
...
void NET_MATRIX::add_faulted_ac_line_to_negative_sequence_network(const AC_LINE& line)
{
   if (line.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_transformers_to_negative_sequence_network()
{
   add_transformers_to_network();
}

void NET_MATRIX::add_loads_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<LOAD*> loads = psdb.get_all_loads();
...
void NET_MATRIX::add_load_to_negative_sequence_network(const LOAD& load)
{
   if (load.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_sources_to_negative_sequence_network()
{
...
void NET_MATRIX::add_generators_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<GENERATOR*> generators = psdb.get_all_generators();
...
void NET_MATRIX::add_generator_to_negative_sequence_network(const GENERATOR& gen)
{
   if (gen.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_wt_generators_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<WT_GENERATOR*> wt_generators = psdb.get_all_wt_generators();
...
void NET_MATRIX::add_wt_generator_to_negative_sequence_network(const WT_GENERATOR& wt_gen)
{
   if (wt_gen.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_pv_units_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<PV_UNIT*> pv_units = psdb.get_all_pv_units();
...
void NET_MATRIX::add_pv_unit_to_negative_sequence_network(const PV_UNIT& pv_unit)
{
   if (pv_unit.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_energy_storages_to_negative_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<ENERGY_STORAGE*> eses = psdb.get_all_energy_storages();
...
void NET_MATRIX::add_energy_storage_to_negative_sequence_network(const ENERGY_STORAGE& es)
{
   return;
}

void NET_MATRIX::add_fixed_shunts_to_negative_sequence_network()
{
   add_fixed_shunts_to_network();
}

void NET_MATRIX::add_ac_lines_to_zero_sequence_network()
{
...
void NET_MATRIX::preprocess_mutual_data()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<MUTUAL_DATA*> mutuals = psdb.get_all_mutual_data();
   size_t n = mutuals.size();
   if (n == 0) {
      return;
   }
   ...
   for (size_t i = 0; i < n; i++) {
      ...
      bool flag = false;
      for (size_t j = 0; j < lineptrs.size(); j++) {
...
void NET_MATRIX::add_ac_line_to_zero_sequence_network(const AC_LINE& line)
{
   if (line.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_faulted_ac_line_to_zero_sequence_network(const AC_LINE& line)
{
   if (line.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_mutual_ac_line_to_zero_sequence_network(const AC_LINE& line)
{
   if (line.get_sequence_parameter_import_flag() == false) {
      return;
   }
   ...
   if (line.get_sending_side_breaker_status() == true or line.get_receiving_side_breaker_status() == true) {
      if (line.is_mutual()) {
         ...
         if (line.get_sending_side_breaker_status() == true and line.get_receiving_side_breaker_status() == true) {
            ...
            for (size_t k = 0; k < Ymutual.size(); k++) {
               complex<double> Y = Ymutual[k];
               AC_LINE* lineptr = pointers[k];
               size_t p = inphno.get_internal_bus_number_of_physical_bus_number(lineptr->get_sending_side_bus());
               size_t q = inphno.get_internal_bus_number_of_physical_bus_number(lineptr->get_receiving_side_bus());
...
void NET_MATRIX::add_transformers_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<TRANSFORMER*> transformers = psdb.get_all_transformers();
...
void NET_MATRIX::add_transformer_to_zero_sequence_network(const TRANSFORMER& trans)
{
...
void NET_MATRIX::add_two_winding_transformer_to_zero_sequence_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or trans.get_winding_breaker_status(SECONDARY_SIDE) == true) {
      size_t primary_bus = trans.get_winding_bus(PRIMARY_SIDE);
      size_t secondary_bus = trans.get_winding_bus(SECONDARY_SIDE);
...
void NET_MATRIX::add_three_winding_transformer_to_zero_sequence_network(const TRANSFORMER& trans)
{
   if (trans.get_winding_breaker_status(PRIMARY_SIDE) == true or
         trans.get_winding_breaker_status(SECONDARY_SIDE) == true or
         trans.get_winding_breaker_status(TERTIARY_SIDE) == true) {
...
void NET_MATRIX::add_loads_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<LOAD*> loads = psdb.get_all_loads();
...
void NET_MATRIX::add_load_to_zero_sequence_network(const LOAD& load)
{
   if (load.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_sources_to_zero_sequence_network()
{
...
void NET_MATRIX::add_generators_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<GENERATOR*> generators = psdb.get_all_generators();
...
void NET_MATRIX::add_generator_to_zero_sequence_network(const GENERATOR& gen)
{
   if (gen.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_wt_generators_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<WT_GENERATOR*> wt_generators = psdb.get_all_wt_generators();
...
void NET_MATRIX::add_wt_generator_to_zero_sequence_network(const WT_GENERATOR& wt_gen)
{
   if (wt_gen.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_pv_units_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<PV_UNIT*> pv_units = psdb.get_all_pv_units();
...
void NET_MATRIX::add_pv_unit_to_zero_sequence_network(const PV_UNIT& pv_unit)
{
   if (pv_unit.get_sequence_parameter_import_flag() == false) {
      return;
   }
...
void NET_MATRIX::add_energy_storages_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<ENERGY_STORAGE*> eses = psdb.get_all_energy_storages();
...
void NET_MATRIX::add_energy_storage_to_zero_sequence_network(const ENERGY_STORAGE& es)
{
   return;
}

void NET_MATRIX::add_fixed_shunts_to_zero_sequence_network()
{
   PF_DATA& psdb = toolkit->get_database();
   vector<FIXED_SHUNT*> shunts = psdb.get_all_fixed_shunts();
...
void NET_MATRIX::add_fixed_shunt_to_zero_sequence_network(const FIXED_SHUNT& shunt)
{
...
void NET_MATRIX::optimize_network_ordering()
{
   initialize_physical_internal_bus_pair();
   build_network_Y_matrix();
...
bool NET_MATRIX::is_condition_ok() const
{
   return true;
}

void NET_MATRIX::initialize_physical_internal_bus_pair()
{
   PF_DATA& psdb = toolkit->get_database();
...
void NET_MATRIX::reorder_physical_internal_bus_pair()
{
   vector<size_t> permutation = network_Y1_matrix.get_reorder_permutation();
...
void NET_MATRIX::check_network_connectivity(bool remove_void_island)
{
   PF_DATA& psdb = toolkit->get_database();
...
vector< vector<size_t> > NET_MATRIX::get_islands_with_internal_bus_number()
{
...
vector< vector<size_t> > NET_MATRIX::get_islands_with_physical_bus_number()
{
...
size_t NET_MATRIX::get_internal_bus_number_of_physical_bus(size_t bus) const
{
   return inphno.get_internal_bus_number_of_physical_bus_number(bus);
}

size_t NET_MATRIX::get_physical_bus_number_of_internal_bus(size_t bus) const
{
   return inphno.get_physical_bus_number_of_internal_bus_number(bus);
}

void NET_MATRIX::report_network_matrix()
{
   ostringstream osstream;
...
void NET_MATRIX::report_decoupled_network_matrix() const
{
   ostringstream osstream;
...
void NET_MATRIX::report_dc_network_matrix() const
{
   ostringstream osstream;
...
void NET_MATRIX::report_dynamic_network_matrix()
{
   ostringstream osstream;
...
void NET_MATRIX::report_positive_sequence_Y_matrix()
{
   ostringstream osstream;
...
void NET_MATRIX::report_negative_sequence_Y_matrix()
{
   ostringstream osstream;
...
void NET_MATRIX::report_zero_sequence_Y_matrix()
{
   ostringstream osstream;
...
void NET_MATRIX::report_network_matrix_common() const
{
   ostringstream osstream;
   ...
   osstream << "Network Y matrix listing finished.";
   toolkit->show_information(osstream);
}


void NET_MATRIX::save_network_Y_matrix(const string& filename, bool export_full_matrix)
{
   ostringstream osstream;
...
void NET_MATRIX::save_decoupled_network_B_matrix_to_file(const string& filename) const
{
   ostringstream osstream;
   ...
   if (file.is_open()) {
      ...
      for (size_t j = 0; j != n; ++j) {
         ...
         for (int k = k_starting; k != k_ending; ++k) {
            ...
            snprintf(buffer, 1000, "%zu,%zu,%zu,%zu,%.14f,%.14f", i, ibus, j, jbus, yp.imag(), yq.imag());
            file << buffer << endl;
            /*file<<i<<","<<ibus<<","<<j<<","<<jbus<<","
...
void NET_MATRIX::save_dc_network_B_matrix_to_file(const string& filename) const
{
   ostringstream osstream;
   ...
   if (file.is_open()) {
      ...
      for (size_t j = 0; j != n; ++j) {
         ...
         for (int k = k_starting; k != k_ending; ++k) {
            ...
            snprintf(buffer, 1000, "%zu,%zu,%zu,%zu,%.14f", i, ibus, j, jbus, y.imag());
            file << buffer << endl;
            /*file<<i<<","<<ibus<<","<<j<<","<<jbus<<","
...
void NET_MATRIX::save_dynamic_network_Y_matrix_to_file(const string& filename)
{
   ostringstream osstream;
...
void NET_MATRIX::save_network_matrix_common(ofstream& file) const
{
   ...
   for (size_t j = 0; j != n; ++j) {
      ...
      for (int k = k_starting; k != k_ending; ++k) {
         ...
         snprintf(buffer, 1000, "%zu,%zu,%zu,%zu,%.14f,%.14f", i, ibus, j, jbus, y.real(), y.imag());
         file << buffer << endl;
         /*file<<i<<","<<ibus<<","<<j<<","<<jbus<<","
...
void NET_MATRIX::save_full_network_matrix_common(ofstream& file) const
{
   size_t i, ibus;
   size_t n = this_Y_matrix_pointer->get_matrix_size();
   complex<double> y;
...
void NET_MATRIX::save_network_Z_matrix_to_file(const string& filename) const
{
   ostringstream osstream;
      ...
      for (size_t j = 0; j != n; ++j) {
         ...
         for (int k = k_starting; k != k_ending; ++k) {
            ...
            if (z.real() != INFINITE_THRESHOLD and z.imag() != INFINITE_THRESHOLD) {
               snprintf(buffer, 1000, "%zu,%zu,%zu,%zu,%.14f,%.14f", i, ibus, j, jbus, z.real(), z.imag());
               file << buffer << endl;
            }
         }
         k_starting = k_ending;
...
void NET_MATRIX::save_positive_sequence_Y_matrix_to_file(const string& filename)
{
   ostringstream osstream;
...
void NET_MATRIX::save_negative_sequence_Y_matrix_to_file(const string& filename)
{
   ostringstream osstream;
...
void NET_MATRIX::save_zero_sequence_Y_matrix_to_file(const string& filename)
{
   ostringstream osstream;
...
void NET_MATRIX::report_physical_internal_bus_number_pair() const
{
   inphno.report();
}

size_t NET_MATRIX::get_memory_usage()
{
   return network_Y1_matrix.get_memory_usage() +
          network_Y2_matrix.get_memory_usage() +
          network_Y0_matrix.get_memory_usage() +

          network_Z1_matrix.get_memory_usage() +
          network_Z2_matrix.get_memory_usage() +
          network_Z0_matrix.get_memory_usage() +

          network_Y1_matrix.get_memory_usage() +

          network_BP_matrix.get_memory_usage() +
          network_BQ_matrix.get_memory_usage() +
          network_DC_B_matrix.get_memory_usage() +

          this_jacobian.get_memory_usage() +

          inphno.get_table_size() * 2 * sizeof(size_t);
}
