...
double api_get_wt_generator_sequence_float_data(size_t bus, char* identifier, char* parameter_name, size_t toolkit_index)
{
...
void api_set_wt_generator_sequence_float_data(size_t bus, char* identifier, char* parameter_name, double value, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    DEVICE_ID did = get_wt_generator_device_id(bus, identifier);
    PF_DATA& psdb = toolkit.get_database();
