...
int api_get_vsc_hvdc_integer_data(char* vsc_name, char* device_type, char* parameter_name, size_t index, size_t toolkit_index)
{
...
void api_set_vsc_hvdc_integer_data(char* vsc_name, char* device_type, char* parameter_name, int value, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
...
double api_get_vsc_hvdc_float_data(char* vsc_name, char* device_type, char* parameter_name, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
        ...
        if(DEVICE_TYPE == "CONVERTER" or DEVICE_TYPE == "VSC")
        {
            if(PARAMETER_NAME == "PREF_MW" or PARAMETER_NAME == "NOMINAL AC ACTIVE POWER IN MW")
            {
                ...
                else
                {
                    ...
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Active power control mode of converter index  %zu  is not VSC_AC_ACTIVE_POWER_CONTORL, please check control mode!", index);
            ...
            if(PARAMETER_NAME == "UDCREF_KV" or PARAMETER_NAME == "NOMINAL DC VOLTAGE IN KV")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Active power control mode of converter index  %zu  is not VSC_DC_VOLTAGE_CONTORL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
            if(PARAMETER_NAME == "UDC_KV" or PARAMETER_NAME == "INITIAL DC VOLTAGE IN KV")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Active power control mode of converter index  %zu  is not VSC_DC_ACTIVE_POWER_VOLTAGE_DROOP_CONTROL and VSC_DC_CURRENT_VOLTAGE_DROOP_CONTROL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
            if(PARAMETER_NAME == "IDC_KA" or PARAMETER_NAME == "INITIAL DC CURRENT IN KA")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Active power control mode of converter index  %zu  is not VSC_DC_CURRENT_VOLTAGE_DROOP_CONTROL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
            if(PARAMETER_NAME == "PDC_MW" or PARAMETER_NAME == "INITIAL DC ACTIVE POWER IN MW")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Active power control mode of converter index  %zu  is not VSC_DC_ACTIVE_POWER_VOLTAGE_DROOP_CONTROL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
            ...
            if(PARAMETER_NAME == "UACREF_PU" or PARAMETER_NAME == "NOMINAL AC VOLTAGE IN PU")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Reactive power control mode of converter index  %zu  is not VSC_AC_VOLTAGE_CONTROL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
            if(PARAMETER_NAME == "QACREF_MVAR" or PARAMETER_NAME == "NOMINAL AC REACTIVE POWER IN MVAR")
            {
                ...
                {
                    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
                    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Reactive power control mode of converter index  %zu  is not VSC_AC_REACTIVE_POWER_CONTROL, please check control mode!", index);
                    toolkit.show_information(buffer);
                    return 0.0;
                }
            }
...
void api_set_vsc_hvdc_float_data(char* vsc_name, char* device_type, char* parameter_name, double value, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
...
const char* api_get_vsc_hvdc_string_data(char* vsc_name, char* device_type, char* parameter_name, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
...
void api_set_vsc_hvdc_string_data(char* vsc_name, char* device_type, char* parameter_name, char* value, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
...
bool api_get_vsc_hvdc_boolean_data(char* vsc_name, char* device_type, char* parameter_name, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
...
void api_set_vsc_hvdc_boolean_data(char* vsc_name, char* device_type, char* parameter_name, bool value, size_t index, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
