...
void show_vsc_hvdc_not_exist_with_api(string name, string api_func, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
...
void show_model_type_not_supported_for_device_with_api(string model_type, DEVICE_ID did, string api_func, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
...
void show_parameter_not_supported_for_area_zone_owner_with_api(string par_name, size_t no, string api_func, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Parameter '%s' is not supported for Area/Zone/Owner %zu with api %s.\n"
             "Nothing will be changed.\n"
             "If return value is expected, 0/0.0/False/EMPTY STRING will be returned",
...
void show_area_zone_owner_not_exist_with_api(size_t no, string api_func, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
    snprintf(buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "Area/Zone/Owner %zu does not exist in database with api %s.\n"
             "Nothing will be changed.\n"
             "If return value is expected, 0/0.0/False/EMPTY STRING will be returned",
             no, api_func.c_str());
    toolkit.show_information(buffer);
...
size_t get_owner_of_nonbus_device(NONBUS_DEVICE* device, string parameter_name, size_t toolkit_index)
{
...
void set_owner_of_nonbus_device(NONBUS_DEVICE* device, string parameter_name, int value, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    string PARAMETER_NAME = string2upper(parameter_name);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
...
double get_owner_fraction_of_nonbus_device(NONBUS_DEVICE* device, string parameter_name, size_t toolkit_index)
{
    string PARAMETER_NAME = string2upper(parameter_name);
...
void set_owner_fraction_of_nonbus_device(NONBUS_DEVICE* device, string parameter_name, double value, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    string PARAMETER_NAME = string2upper(parameter_name);
    char buffer[STEPS_MAX_TEMP_CHAR_BUFFER_SIZE];
