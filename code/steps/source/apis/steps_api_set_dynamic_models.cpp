...
void api_set_dynamic_model(char* model_string, char* file_type, size_t toolkit_index)
{
    ...
    if(FileType=="PSSE" or FileType=="PSS/E")
	{
	    //PSSE_IMEXPORTER importer(toolkit);
         ...
         toolkit.importer.load_one_model(data);
	}
	else
    {
        if(FileType=="BPA")
...
void api_disable_generator_related_model(size_t bus, char* identifier, char* model_type, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
...
void api_enable_generator_related_model(size_t bus, char* identifier, char* model_type, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
	snprintf(toolkit.steps_char_buffer, STEPS_MAX_TEMP_CHAR_BUFFER_SIZE, "%s", "");
    DEVICE_ID did = get_generator_device_id(bus, identifier);
