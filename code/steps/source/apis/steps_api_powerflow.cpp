...
size_t api_get_powerflow_solver_integer_parameter(char* parameter_name, size_t toolkit_index)
{
    ...
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
...
void api_set_powerflow_solver_float_parameter(char* parameter_name, double value, size_t toolkit_index)
{
    ...
    if(PARAMETER_NAME=="MAX ACTIVE POWER IMBALANCE IN MW")
    {
        solver.set_active_power_mismatch_threshold(value);
        return;
    }
    if(PARAMETER_NAME=="MAX ACTIVE POWER IMBALANCE IN PU")
    {
        solver.set_active_power_mismatch_threshold(value*psdb.get_system_base_power_in_MVA());
        return;
    }
    if(PARAMETER_NAME=="MAX REACTIVE POWER IMBALANCE IN MVAR")
    {
        solver.set_reactive_power_mismatch_threshold(value);
        return;
    }
    if(PARAMETER_NAME=="MAX REACTIVE POWER IMBALANCE IN PU")
    {
        solver.set_reactive_power_mismatch_threshold(value*psdb.get_system_base_power_in_MVA());
        return;
    }
    if(PARAMETER_NAME=="ITERATION ACCELERATOR")
    {
...
bool api_is_nan_detected_in_powerflow_solution(size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
...
void api_save_powerflow_result(char* file, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
    solver.save_powerflow_result(file);
}
...
void api_save_extended_powerflow_result(char* file, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
    solver.save_extended_powerflow_result(file);
}
...
void api_save_jacobian_matrix(char* file, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
    solver.save_jacobian_matrix(file);
}
...
void api_save_network_Y_matrix(char* file, bool export_full, size_t toolkit_index)
{
    iSTEPS& toolkit = get_toolkit(toolkit_index);
    PF_SOLVER& solver = toolkit.get_powerflow_solver();
    NET_MATRIX& network_matrix = solver.get_network_matrix();
    network_matrix.save_network_Y_matrix(file, export_full);
}
