...
double api_get_fixed_shunt_float_data(size_t bus, char* identifier, char* parameter_name, size_t toolkit_index)
{
    ...
    if(shuntptr!=NULL)
    {
        ...
        if(PARAMETER_NAME=="ANGLE_RAD" or PARAMETER_NAME=="ANGLE IN RAD" or PARAMETER_NAME=="POSITIVE ANGLE IN RAD")
            return arg(shuntptr->get_positive_sequence_complex_current_in_pu());
        if(PARAMETER_NAME=="ANGLE_DEG" or PARAMETER_NAME=="ANGLE IN DEG" or PARAMETER_NAME=="POSITIVE ANGLE IN DEG")
            return rad2deg(arg(shuntptr->get_positive_sequence_complex_current_in_pu()));
        if(PARAMETER_NAME=="NEGATIVE_ANGLE_RAD" or PARAMETER_NAME=="NEGATIVE ANGLE IN RAD")
            return arg(shuntptr->get_negative_sequence_complex_current_in_pu());
        if(PARAMETER_NAME=="NEGATIVE_ANGLE_DEG" or PARAMETER_NAME=="NEGATIVE ANGLE IN DEG")
            return rad2deg(arg(shuntptr->get_negative_sequence_complex_current_in_pu()));
        if(PARAMETER_NAME=="ZERO_ANGLE_RAD" or PARAMETER_NAME=="ZERO ANGLE IN RAD")
            return arg(shuntptr->get_zero_sequence_complex_current_in_pu());
        if(PARAMETER_NAME=="ZERO_ANGLE_DEG" or PARAMETER_NAME=="ZERO ANGLE IN DEG")
            return rad2deg(arg(shuntptr->get_zero_sequence_complex_current_in_pu()));
