...
void STEPS_IMEXPORTER::load_sequence_data_from_steps_vector(vector<vector<vector<string> > >& data)
{
    ...
    if(splitted_sseq_data_in_ram.size()==0)
    {
        ...
        osstream<<"No data in the given iSTEPS sequence vector <splitted_sseq_data_in_ram>."<<endl
...
void STEPS_IMEXPORTER::load_zero_seq_mutual_impedance_data()
{
    ...
    size_t n = 0;
    size_t bus_i=0, bus_j=0, bus_p=0, bus_q=0;
...
void STEPS_IMEXPORTER::load_zero_seq_transformer_data()
{
    ...
    size_t n = 0;
    size_t bus_i = 0, bus_j = 0, bus_k = 0;
    string ID = "1";
...
void STEPS_IMEXPORTER::load_two_winding_transformer_zero_seq_data(TRANSFORMER& trans, vector<string> data)
{
    ...
    size_t units_of_non_grounding_impedance, units_of_grounding_impedance, connection_code;
...
void STEPS_IMEXPORTER::load_vsc_hvdc_seq_data()
{
    ...
    for(size_t i=0; i!=ndata; ++i)
    {
        ...
        size_t ncon = vsc_hvdc_ptr->get_converter_count();
        ...
        for(size_t j=0; j<ncon; ++j)
        {
            ...
            size_t index = vsc_hvdc_ptr->get_converter_index_with_ac_bus(bus);
