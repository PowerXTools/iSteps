...
{
    PF_DATA& psdb = default_toolkit.get_database();
    psdb.set_max_bus_number(number);
}
...
void CCT_SEARCHER::prepare_generators_in_islands(DYN_SIMULATOR& simulator)
{
    generators_in_islands.clear();

    PF_DATA& psdb = default_toolkit.get_database();

    NET_MATRIX& network_matrix = default_toolkit.get_network_matrix();
...
void CCT_SEARCHER::apply_fault(DYN_SIMULATOR& simulator)
{
...
void CCT_SEARCHER::clear_fault(DYN_SIMULATOR& simulator)
{
    DEVICE_ID did = get_fault_device();
    size_t bus = get_fault_side_bus();
    double location = get_fault_location_to_fault_side_bus_in_pu();
...
//bool CCT_SEARCHER::check_if_system_is_stable(DYN_SIMULATOR& simulator) const
