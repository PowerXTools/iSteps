#include "header/toolkit/contingency_screener.h"
...
{
    default_toolkit.get_database().set_max_bus_number(10000);
...
{
    default_toolkit.get_database().set_max_bus_number(number);
}

size_t CONTINGENCY_SCREENER::get_power_system_database_maximum_bus_number() const
{
    return default_toolkit.get_database().get_allowed_max_bus_number();
}
...
bool CONTINGENCY_SCREENER::perform_simulation_with_clearing_time(double clearing_time)
{
    ...
    PF_SOLVER solver(default_toolkit);
    solver.set_active_power_mismatch_threshold(0.00001);
    solver.set_reactive_power_mismatch_threshold(0.00001);
...
void CONTINGENCY_SCREENER::set_meters(DYN_SIMULATOR& simulator)
{
...
void CONTINGENCY_SCREENER::apply_fault(DYN_SIMULATOR& simulator)
{
...
void CONTINGENCY_SCREENER::clear_fault(DYN_SIMULATOR& simulator)
{
    DEVICE_ID did = get_fault_device();
    size_t bus = get_fault_side_bus();
    double location = get_fault_location_to_fault_side_bus_in_pu();
...
bool CONTINGENCY_SCREENER::check_if_system_is_stable(DYN_SIMULATOR& simulator) const
{
