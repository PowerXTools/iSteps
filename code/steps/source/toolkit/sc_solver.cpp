...
SC_SOLVER::SC_SOLVER(iSTEPS& toolkit)
{
...
iSTEPS& SC_SOLVER::get_toolkit() const
{
    return *toolkit;
}

void SC_SOLVER::reset_data()
{
...
NET_MATRIX& SC_SOLVER::get_network_matrix()
{
    return toolkit->get_network_matrix();
}


void SC_SOLVER::initialize_short_circuit_solver()
{
...
void SC_SOLVER::show_short_circuit_solver_configuration()
{
...
void SC_SOLVER::store_bus_initial_voltage_before_short_circuit()
{
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::restore_bus_initial_voltage()
{
    PF_DATA& psdb = toolkit->get_database();
    vector<BUS*> buses = psdb.get_all_buses();
    size_t n = buses.size();
    ...
    {
        BUS* busptr = buses[i];
        size_t bus = busptr->get_bus_number();
...
complex<double> SC_SOLVER::get_initial_voltage_of_bus_before_short_circuit(size_t bus)
{
...
void SC_SOLVER::update_internal_voltage_of_all_generators_and_wt_generators()
{
    update_internal_voltage_of_all_generators();
    update_internal_voltage_of_all_wt_generators();
}

void SC_SOLVER::update_internal_voltage_of_all_generators()
{
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::update_internal_voltage_of_all_wt_generators()
{
    PF_DATA& psdb = toolkit->get_database();
    vector<WT_GENERATOR*> wt_gens = psdb.get_all_wt_generators();

    size_t n = wt_gens.size();
    for(size_t i=0; i<n; i++)
        wt_gens[i]->update_internal_voltage_for_short_circuit_solver();
}

void SC_SOLVER::update_all_motor_load_data()
{
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::updata_all_wt_generator_motor_data()
{
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::update_node_voltages_with_devices_equivalent_to_source()
{
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::add_generators_to_injection_current_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        size_t busn = gen->get_generator_bus();
        ...
        size_t b = get_internal_bus_number_of_physical_bus(busn);
        injection_current_vector_with_internal_order[b] += E/Z;
    }
}
void SC_SOLVER::add_wt_generators_to_injection_current_vector()
{
//    PF_DATA& psdb = toolkit->get_database();
//    double sbase = psdb.get_system_base_power_in_MVA();
//    GENERATOR_REACTANCE_OPTION gen_X_option = get_generator_reactance_option();
...
//        size_t busn = wt_gen->get_generator_bus();
...
void SC_SOLVER::add_wt_generators_to_injection_current_vector_for_iterative_method()
{
    PF_DATA& psdb = toolkit->get_database();
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        if(wt_gen->get_status())
        {
            ...
            size_t b =  get_internal_bus_number_of_physical_bus(bus);
...
void SC_SOLVER::add_constant_speed_wt_generator_to_vector(WT_GENERATOR& wt_gen)
{
...
void SC_SOLVER::add_direct_driven_wt_generator_to_vector(WT_GENERATOR& wt_gen)
{
    // as current source
    if(wt_gen.get_status())
    {
...
void SC_SOLVER::add_doubly_fed_wt_generator_to_vector(WT_GENERATOR& wt_gen)
{
    if(wt_gen.get_status())
    {
        PF_DATA& psdb = toolkit->get_database();
	...
	void SC_SOLVER::add_pv_units_to_injection_current_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        if(pv_unit->get_status())
        {
            ...
            size_t busn = pv_unit->get_source_bus();
...
void SC_SOLVER::add_pv_units_to_injection_current_vector_for_iterative_method()
{
    PF_DATA& psdb = toolkit->get_database();
//    double sbase = psdb.get_system_base_power_in_MVA();
    vector<PV_UNIT*> pv_units = psdb.get_all_pv_units();
...
void SC_SOLVER::add_energy_storages_to_injection_current_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();

    vector<ENERGY_STORAGE*> energy_storages = psdb.get_all_energy_storages();
    size_t n = energy_storages.size();
    for(size_t i=0; i<n; i++)
    {
        ...
        size_t busn = energy_storage->get_source_bus();
...
void SC_SOLVER::add_motor_load_to_injection_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    vector<LOAD*> loads = psdb.get_all_loads();
    size_t n = loads.size();
...
void SC_SOLVER::add_2t_lcc_hvdcs_to_injection_current_vector()
{
    ;
}

void SC_SOLVER::add_vsc_hvdcs_to_injection_current_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        size_t nn = vsc_hvdc->get_converter_count();

        for(size_t j=0; j<nn; j++)
        {
            size_t busn = vsc_hvdc->get_converter_ac_bus(j);
...
void SC_SOLVER::update_voltages_with_current_vector()
{
    PF_DATA& psdb = toolkit->get_database();
    ...
    {
        BUS* bus = buses[i];
        size_t busn = bus->get_bus_number();
...
void SC_SOLVER::set_generator_reactance_option(GENERATOR_REACTANCE_OPTION gen_X_option)
{
...
GENERATOR_REACTANCE_OPTION SC_SOLVER::get_generator_reactance_option()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    return network_matrix.get_generator_reactance_option();
}

void SC_SOLVER::set_units_of_currents_and_voltages(UNITS_OPTION option)
{
    units_of_currents_and_voltages = option;
}
UNITS_OPTION SC_SOLVER::get_units_of_currents_and_voltages()
{
    return units_of_currents_and_voltages;
}

void SC_SOLVER::set_coordinates_of_currents_and_voltages(COORDINATES_OPTION option)
{
    coordindates_of_currents_and_volatges = option;
}
COORDINATES_OPTION SC_SOLVER::get_coordinates_of_currents_and_voltages()
{
    return coordindates_of_currents_and_volatges;
}

void SC_SOLVER::set_max_iteration_cout_for_iterative_method(size_t n)
{
    max_iteration_count = n;
}

size_t SC_SOLVER::get_max_iteration_count_for_iterative_method()
{
    return max_iteration_count;
}

void SC_SOLVER::set_voltage_threshold_for_iterative_method(double threshold)
{
    voltage_threshold_in_pu_for_iterative_method = threshold;
}

double SC_SOLVER::get_voltage_threshold_for_iterative_method()
{
    return voltage_threshold_in_pu_for_iterative_method;
}

void SC_SOLVER::set_consider_load_logic(bool logic)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.set_consider_load_logic(logic);
}
bool SC_SOLVER::get_consider_load_logic()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    return network_matrix.get_consider_load_logic();
}
void SC_SOLVER::set_consider_motor_load_logic(bool logic)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.set_consider_motor_load_logic(logic);
}
bool SC_SOLVER::get_consider_motor_load_logic()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    return network_matrix.get_consider_motor_load_logic();
}

void SC_SOLVER::set_option_of_DC_lines(DC_LINES_OPTION option)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.set_option_of_dc_lines(option);
}

DC_LINES_OPTION SC_SOLVER::get_option_of_DC_lines()
{
    NET_MATRIX& network_matrix = get_network_matrix();
...
void SC_SOLVER::set_bus_fault(size_t bus, FAULT_TYPE type, const complex<double>& fault_shunt)
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::set_ac_line_fault(const DEVICE_ID& line_id, size_t side_bus, double location, FAULT_TYPE fault_type, const complex<double>& fault_shunt)
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
...
string SC_SOLVER::get_fault_information()
{
    char infor[200];
    if(is_bus_fault())
    {
...
bool SC_SOLVER::is_ac_line_fault()
{
...
FAULT_TYPE SC_SOLVER::get_fault_type()
{
    return fault.get_fault_type();
}

void SC_SOLVER::build_sequence_network()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.build_sequence_network_Y_matrix();
}

size_t SC_SOLVER::get_internal_bus_number_of_physical_bus(size_t bus)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    return network_matrix.get_internal_bus_number_of_physical_bus(bus);
}
size_t SC_SOLVER::get_physical_bus_number_of_internal_bus(size_t bus)
{
    NET_MATRIX& network_metrix = get_network_matrix();
    return network_metrix.get_physical_bus_number_of_internal_bus(bus);
}


void SC_SOLVER::calculate_and_store_equivalent_impedance_between_bus_and_fault_place()
{
    ...
    if(is_bus_fault())
    {
        size_t bus = faulted_bus_pointer->get_bus_number();
        ...
        size_t internal_bus = get_internal_bus_number_of_physical_bus(bus);
    ...
    else if(is_ac_line_fault())
    {
        ...
        size_t ibus = faulted_ac_line_pointer->get_sending_side_bus();
        size_t jbus = faulted_ac_line_pointer->get_receiving_side_bus();
        size_t internal_ibus = get_internal_bus_number_of_physical_bus(ibus);
        size_t internal_jbus = get_internal_bus_number_of_physical_bus(jbus);
        ...
        else // faulted location at mutual line
        {
            ...
            for(size_t s=0; s<n; s++)
            {
                ...
                for(size_t k=0; k<lineptrs_Ymutual.size(); k++)
                {
                    complex<double> Ytemp = Ymutual[k];
                    AC_LINE* lineptr = lineptrs_Ymutual[k];
                    size_t pbus = lineptr->get_sending_side_bus();
                    size_t qbus = lineptr->get_receiving_side_bus();
                ...
                // complex<double> Ipq = 0.0;
                 for(size_t k=0; k<mutual_lineptrs.size(); k++)
                 {
                    complex<double> Ipq = 0.0;
                    ...
                    for(size_t t=0; t<lineptrs_Ymutual_pq.size(); t++)
                    {
                        complex<double> Ytemp = Ymutual_pq[t];
                        AC_LINE* linep = lineptrs_Ymutual_pq[t];
                        size_t pbus = linep->get_sending_side_bus();
                        size_t qbus = linep->get_receiving_side_bus();
                        vector<complex<double> > Z0p = network_matrix.get_zero_sequence_complex_impedance_of_column_with_physical_bus(pbus);
                        vector<complex<double> > Z0q = network_matrix.get_zero_sequence_complex_impedance_of_column_with_physical_bus(qbus);
            ...
            for(size_t k=1; k<lineptrs_Ymutual.size(); k++)
            {
                AC_LINE* lineptr = lineptrs_Ymutual[k];
            ...
            for(size_t k=1; k<lineptrs_Ymutual.size(); k++)
            {
                ...
                for(size_t t=0; t<pq_mutuals.size(); t++)
                {
                    ...
                    {
                        size_t index=0;
                        for(size_t tt=k; tt<lineptrs_Ymutual.size(); tt++)
                        {
...
complex<double> SC_SOLVER::get_positive_sequence_equivalent_mutual_impedance_in_pu_between_bus_to_fault_place(size_t bus)
{
    size_t b = get_internal_bus_number_of_physical_bus(bus);
    return Z1_between_internal_bus_to_fault_place[b];
}
complex<double> SC_SOLVER::get_negative_sequence_equivalent_mutual_impedance_in_pu_between_bus_to_fault_place(size_t bus)
{
    size_t b = get_internal_bus_number_of_physical_bus(bus);
    return Z2_between_internal_bus_to_fault_place[b];
}
complex<double> SC_SOLVER::get_zero_sequence_equivalent_mutual_impedance_in_pu_between_bus_to_fault_place(size_t bus)
{
    size_t b = get_internal_bus_number_of_physical_bus(bus);
    return Z0_between_internal_bus_to_fault_place[b];
}

void SC_SOLVER::solve()
{
    ostringstream osstream;
    if(is_fault())
    {
...
void SC_SOLVER::solve_with_iteration()
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    size_t bus_count = psdb.get_bus_count();
    ...
    size_t max_iteration = get_max_iteration_count_for_iterative_method();
...
complex<double> SC_SOLVER::get_initial_voltage_of_fault_location_before_short_circuit()
{
    complex<double> Uf;
    if(is_bus_fault())
...
void SC_SOLVER::update_bus_sequence_voltage()
{
    PF_DATA& psdb = toolkit->get_database();
    ...
    {
        BUS& bus = *buses[i];
        size_t busnum = bus.get_bus_number();
...
void SC_SOLVER::solve_fault_current(complex<double> Uf)
{
...
complex<double> SC_SOLVER::get_positive_sequence_fault_current_in_pu()
{
    return If1;
}
complex<double> SC_SOLVER::get_negative_sequence_fault_current_in_pu()
{
    return If2;
}
complex<double> SC_SOLVER::get_zero_sequence_fault_current_in_pu()
{
    return If0;
}
complex<double> SC_SOLVER::get_positive_sequence_fault_current_in_kA()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_negative_sequence_fault_current_in_kA()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_zero_sequence_fault_current_in_kA()
{
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_initial_voltage_of_faulted_ac_line_point_before_short_circuit()
{
    double fault_location = faulted_ac_line_pointer->get_fault_location_of_fault(0);
    size_t ibus = faulted_ac_line_pointer->get_sending_side_bus();
    size_t jbus = faulted_ac_line_pointer->get_receiving_side_bus();
...
complex<double> SC_SOLVER::get_positive_sequence_equivalent_fault_admittance()
{
    complex<double> U1 = get_positive_sequence_voltage_of_fault_point_in_pu();
    return If1/U1;
}

complex<double> SC_SOLVER::get_positive_sequence_short_circuit_capacity_in_pu()
{
    complex<double> Uf1 = get_positive_sequence_voltage_of_fault_point_in_pu();
    return If1*Uf1;
}

complex<double> SC_SOLVER::get_negative_sequence_short_circuit_capacity_in_pu()
{
    complex<double> Uf2 = get_negative_sequence_voltage_of_fault_point_in_pu();
    return If2*Uf2;
}
complex<double> SC_SOLVER::get_zero_sequence_short_circuit_capacity_in_pu()
{
    complex<double> Uf0 = get_zero_sequence_voltage_of_fault_point_in_pu();
    return If0*Uf0;
}

void SC_SOLVER::show_contributions_of_fault_current_with_ac_line_fault()
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    ...
    else
    {
        ...
        for(size_t i=0; i<lineptrs_of_mutual_with_line_fault.size(); i++)
        {
    ...
    toolkit->show_information(get_formatted_sequence_data(ibus,id, I1if, I2if, I0if));
    toolkit->show_information(get_formatted_phase_data(ibusptr->get_bus_name(), Vbase,I1if, I2if, I0if));
    ...
    toolkit->show_information(get_formatted_sequence_data(jbus,id, I1jf, I2jf, I0jf));
    toolkit->show_information(get_formatted_phase_data(ibusptr->get_bus_name(),Vbase,I1jf, I2jf, I0jf));
}

complex<double> SC_SOLVER::get_positive_sequence_voltage_of_fault_point_in_pu()
{
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_negative_sequence_voltage_of_fault_point_in_pu()
{
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_zero_sequence_voltage_of_fault_point_in_pu()
{
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_positive_sequence_voltage_of_fault_point_in_kV()
{
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_negative_sequence_voltage_of_fault_point_in_kV()
{
    if(is_bus_fault())
...
complex<double> SC_SOLVER::get_zero_sequence_voltage_of_fault_point_in_kV()
{
    if(is_bus_fault())
...
COMPLEX3 SC_SOLVER::convert_sequence_data_to_phase_data(complex<double> F1, complex<double> F2, complex<double> F0)
{
...
string SC_SOLVER::get_formatted_sequence_data(size_t bus,string ID,complex<double> F1, complex<double>F2, complex<double> F0, bool to_file)
{
    ostringstream osstream;
    string msg = (ID=="")?"":"CKT ";
    string s = (to_file)?",":"";
...
string SC_SOLVER::get_formatted_sequence_data(string str, complex<double> F1, complex<double> F2, complex<double> F0, bool to_file)
{
...
string SC_SOLVER::get_formatted_phase_data(string busname,double Vbase, complex<double> F1, complex<double>F2, complex<double> F0, bool to_file)
{
    ostringstream osstream;
    string s = (to_file)?",":"";
...
string SC_SOLVER::get_formatted_phase_data(string str, complex<double> F1, complex<double> F2, complex<double> F0, bool to_file)
{
    string s = (to_file)?",":"";
...
void SC_SOLVER::show_voltage_table_header()
{
    ostringstream osstream;
...
void SC_SOLVER::show_current_table_header()
{
    ostringstream osstream;
...
void SC_SOLVER::show_short_circuit_result()
{
    if(is_bus_fault())
...
void SC_SOLVER::show_short_circuit_with_bus_fault()
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
    ...
    toolkit->show_information(get_formatted_sequence_data(busnum, "", V1, V2, V0));
    toolkit->show_information(get_formatted_phase_data(busname, Vbase , V1, V2, V0));
    ...
    toolkit->show_information(osstream);
    toolkit->show_information(get_formatted_sequence_data(busnum, "", I1, I2, I0));
    toolkit->show_information(get_formatted_phase_data(busname, Vbase, I1, I2, I0));
...
void SC_SOLVER::show_contributions_of_fault_current_with_bus_fault()
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();

    size_t fault_bus = faulted_bus_pointer->get_bus_number();
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        toolkit->show_information(get_formatted_sequence_data(current_from_bus, line.get_identifier(), I1, I2, I0));
        toolkit->show_information(get_formatted_phase_data(busname, busptr->get_base_voltage_in_kV(), I1, I2, I0));
    }
    ...
    for(size_t i=0; i<n; i++)
    {
        ...
        if(trans.is_two_winding_transformer())
        {
            ...
            toolkit->show_information(get_formatted_sequence_data(current_from_bus,trans.get_identifier(), I1, I2, I0));
            toolkit->show_information(get_formatted_phase_data(busname, busptr->get_base_voltage_in_kV(), I1, I2, I0));
        }
        else
        {
            ...
            toolkit->show_information(get_formatted_sequence_data(osstream.str(), I1, I2, I0));
            toolkit->show_information(get_formatted_phase_data("+", I1, I2, I0));
        }
    }
...
void SC_SOLVER::show_short_circuit_with_ac_line_fault()
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
...
complex<double> SC_SOLVER::get_positive_sequence_voltage_at_ac_line_fault_location_in_pu()
{
...
complex<double> SC_SOLVER::get_positive_sequence_voltage_at_ac_line_fault_location_in_kV()
{
...
complex<double> SC_SOLVER::get_negative_sequence_voltage_at_ac_line_fault_location_in_pu()
{
...
complex<double> SC_SOLVER::get_negative_sequence_voltage_at_ac_line_fault_location_in_kV()
{
...
complex<double> SC_SOLVER::get_zero_sequence_voltage_at_ac_line_fault_location_in_pu()
{
...
complex<double> SC_SOLVER::get_zero_sequence_voltage_at_ac_line_fault_location_in_kV()
{
...
complex<double> SC_SOLVER::get_positive_sequence_thevenin_impedance_at_fault_in_pu()
{
    return Z1;
}
complex<double> SC_SOLVER::get_negative_sequence_thevenin_impedance_at_fault_in_pu()
{
    return Z2;
}
complex<double> SC_SOLVER::get_zero_sequence_thevenin_impedance_at_fault_in_pu()
{
    return Z0;
}

void SC_SOLVER::save_short_circuit_result_to_file(const string& filename)
{
    if(is_bus_fault())
...
void SC_SOLVER::save_short_circuit_result_to_file_with_bus_fault(const string& filename)
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    UNITS_OPTION units = get_units_of_currents_and_voltages();
...
void SC_SOLVER::save_short_circuit_result_to_file_with_ac_line_fault(const string& filename)
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
    double sbase = psdb.get_system_base_power_in_MVA();
...
void SC_SOLVER::save_extended_short_circuit_result_to_file(const string& filename)
{
    ostringstream osstream;
    PF_DATA& psdb = toolkit->get_database();
...
void SC_SOLVER::report_positive_sequence_Y_matrix()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.report_positive_sequence_Y_matrix();
}
void SC_SOLVER::report_negative_sequence_Y_matrix()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.report_negative_sequence_Y_matrix();
}
void SC_SOLVER::report_zero_sequence_Y_matrix()
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.report_zero_sequence_Y_matrix();
}

void SC_SOLVER::save_positive_sequence_Y_matrix_to_file(const string& filename)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.save_positive_sequence_Y_matrix_to_file(filename);
}

void SC_SOLVER::save_negative_sequence_Y_matrix_to_file(const string& filename)
{
    NET_MATRIX& network_matrix = get_network_matrix();
    network_matrix.save_negative_sequence_Y_matrix_to_file(filename);
}

void SC_SOLVER::save_zero_sequence_Y_matrix_to_file(const string& filename)
{
    NET_MATRIX& network_matrix = get_network_matrix();
