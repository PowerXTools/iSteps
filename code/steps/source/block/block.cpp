...
BLOCK::BLOCK(iSTEPS& toolkit)
{
    set_toolkit(toolkit);
    set_limiter_type(NO_LIMITER);
    set_automatic_large_stepsize(false);
...
void BLOCK::set_toolkit(iSTEPS& toolkit)
{
    this->toolkit = (&toolkit);
}

iSTEPS& BLOCK::get_toolkit() const
{
    return *toolkit;
}
...
void BLOCK::set_automatic_large_stepsize(bool flag)
{
    automatic_large_time_step_logic = flag;
}
...
void BLOCK::set_state_index(size_t index)
{
    state_index = index;
}

size_t BLOCK::get_state_index() const
{
    return state_index;
}
