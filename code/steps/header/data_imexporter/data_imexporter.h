...
{
    public:
        DATA_IMEXPORTER(iSTEPS& toolkit);
        ...
        virtual bool load_powerflow_data(string pf_source) = 0;
        ...
        const vector<size_t>& get_ordered_buses() const;
    ...
    private:
        ...
        vector<size_t> get_all_generator_buses();
        vector<size_t> get_all_wt_generator_buses();
        vector<size_t> get_all_pv_unit_buses();
        vector<size_t> get_all_energy_storage_buses();
        vector<size_t> get_all_load_buses();
        vector<size_t> get_all_2t_lcc_hvdc_buses();
        vector<size_t> get_all_fixed_shunt_buses();
        void append_buses_to_ordered_buses(const vector<size_t>& buses, set<size_t>& existing_buses);
        ...
        vector<size_t> ordered_buses_to_export;
};
