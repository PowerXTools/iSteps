...
class WIND_TURBINE_POWER_SPEED_LOOKUP_TABLE
{
    ...
    */
    ...
    public:
        ...
        double get_power_of_record(size_t i) const;
        double get_speed_of_record(size_t i) const;
    private:
        void copy_from_const_table(const WIND_TURBINE_POWER_SPEED_LOOKUP_TABLE& table);
        size_t get_valid_record_size() const;
        void reorder_records();
        bool is_power_record_exist(double power) const;
        size_t get_the_last_record_with_power_less_than_or_equal_to(double power);
