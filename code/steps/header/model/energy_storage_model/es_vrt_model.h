#ifndef ES_VRT_MODEL_H
#define ES_VRT_MODEL_H

#include "header/model/energy_storage_model/es_model.h"
#include <complex>
class ES_VRT_MODEL : public ES_MODEL
{
    /*
    ES voltage ride through control model
    General inputs:
        vterm: ES terminal or remote AC voltage
    General output:
        ipcmd: active current command in pu
        iqcmd: reactive current command in pu
    */
    public:
        ES_VRT_MODEL(iSTEPS& toolkit);
        virtual ~ES_VRT_MODEL();
    public: // pe elctricla control common
        ...
        complex<double> get_energy_storage_terminal_generation_in_MVA() const;
        complex<double> get_energy_storage_terminal_generation_in_pu_based_on_mbase() const;
        complex<double> get_energy_storage_terminal_complex_current_in_pu() const;
        double get_energy_storage_terminal_current_in_pu() const;
