...
class ESCVY : public PV_CONVERTER_MODEL
{
    public:
        ESCVY(iSTEPS& toolkit);
        ESCVY(const ESCVY& model);
        virtual ~ESCVY();
        virtual ESCVY& operator=(const ESCVY&);
    ...
    private:
        void copy_from_const_model(const ESCVY& model);
